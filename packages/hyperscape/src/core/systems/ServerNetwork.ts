...
import type { ChatMessage, World, WorldOptions } from '../../types/index';
...
// Type definitions
// User interface moved below with DatabaseInterface
...
interface DatabaseInterface {
  (table: string): {
    where: (key: string, value: string) => {
      first: () => Promise<{ value?: string }>;
      update: (data: Record<string, unknown>) => Promise<void>;
    };
    insert: (data: Record<string, unknown>) => {
      onConflict: (key: string) => {
        merge: (data: Record<string, unknown>) => Promise<void>;
      };
    } & Promise<void>;
  } & Promise<unknown[]>;
}
...
export class ServerNetwork extends System {
  ...
  isClient: boolean;
  queue: QueueItem[];
  db!: DatabaseInterface; // Database instance (Knex) - initialized in init()
  spawn: SpawnData;
  maxUploadSize: number;

  // Handler method registry
  private handlers: { [method: string]: (socket: Socket, data: unknown) => void | Promise<void> } = {};
  ...
  constructor(world: World) {
    ...
    this.handlers['chatAdded'] = this.onChatAdded.bind(this) as (socket: Socket, data: unknown) => Promise<void>;
    this.handlers['command'] = this.onCommand.bind(this) as (socket: Socket, data: unknown) => Promise<void>;
    this.handlers['entityModified'] = this.onEntityModified.bind(this) as (socket: Socket, data: unknown) => void;
    this.handlers['entityEvent'] = this.onEntityEvent.bind(this) as (socket: Socket, data: unknown) => void;
    this.handlers['entityRemoved'] = this.onEntityRemoved.bind(this) as (socket: Socket, data: unknown) => void;
    this.handlers['settings'] = this.onSettings.bind(this);
  }

  async init(options: WorldOptions): Promise<void> {
    this.db = options.db as DatabaseInterface;
    if (!this.db) {
  ...
  async start(): Promise<void> {
    ...
    if (entities && Array.isArray(entities)) {
      for (const entity of entities) {
        const data = JSON.parse((entity as { data: string }).data);
        data.state = {};
  ...
  async onConnection(ws: WebSocket, params: ConnectionParams): Promise<void> {
    try {
      // check player limit
      ...
      const socket = new Socket({ 
        id: user.id, 
        ws: ws as NodeWebSocket, 
        network: this as unknown as NetworkWithSocket 
      });
      ...
      const addedEntity = this.world.entities.add ? this.world.entities.add(
        {
          id: user.id,
          type: 'player',
          position: this.spawn.position.slice() as [number, number, number],
          quaternion: this.spawn.quaternion.slice() as [number, number, number, number],
