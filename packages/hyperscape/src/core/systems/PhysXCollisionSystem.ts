...
// Access THREE.js through globalThis like the utilities do
const globalWithTHREE = globalThis as typeof globalThis & { THREE: typeof import('three') }
const ThreeJS = globalWithTHREE.THREE!
...
export class PhysXCollisionSystem extends System {
  ...
  private initializeRaycastPool(): void {
    for (let i = 0; i < PhysXCollisionSystem.RAY_POOL_SIZE; i++) {
      this.raycastPool.push(new ThreeJS.Raycaster());
    }
  }
  ...
  private async performPhysXRaycast(x: number, z: number, startHeight: number = 500): Promise<number> {
    ...
    const origin = toTHREEVector3(new ThreeJS.Vector3(x, startHeight, z));
    const direction = toTHREEVector3(new ThreeJS.Vector3(0, -1, 0));
