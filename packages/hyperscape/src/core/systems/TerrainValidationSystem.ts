...
import type { MobSpawnerSystem } from '../../rpg/systems/MobSpawnerSystem';
import type { RPGResourceSystem as _RPGResourceSystem, RPGResourceSystem } from '../../rpg/systems/RPGResourceSystem';
import { TerrainSystem } from './TerrainSystem';
import type { MobSpawnStats } from '../../rpg/types/core';
...
export class TerrainValidationSystem extends System {
  ...
  private async validateMinimumDistances(): Promise<void> {
    try {
      ...
      const resourceSystem = this.world.getSystem<RPGResourceSystem>('RPGResourceSystem');
      if (resourceSystem) {
      ...
      if (mobSystem && mobSystem.getSpawnedMobs) {
        ...
        Array.from(spawnedMobs.entries()).forEach(([mobId, entityId]) => {
