...
export class EntityCullingSystem extends System {
  ...
  private updateEntityCulling(): void {
    ...
    const allEntities = (entityManager as unknown as { getAllEntities(): Entity[] }).getAllEntities();
  ...
  forceUpdateEntityLOD(entityId: string): void {
    ...
    const entity = (entityManager as unknown as { getEntity(id: string): Entity | null }).getEntity(entityId);
    if (entity) {
