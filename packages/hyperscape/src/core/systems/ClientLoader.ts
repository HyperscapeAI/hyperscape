...
import type { PxVec3, PxQuat, PxTransform } from '../../types/physx'
...
function nodeToINode(node: Node): INode {
  ...
  return {
    ...
    transform: {
      p: { x: node.position.x, y: node.position.y, z: node.position.z } as unknown as PxVec3,
      q: { x: node.quaternion.x, y: node.quaternion.y, z: node.quaternion.z, w: node.quaternion.w } as unknown as PxQuat
    } as unknown as PxTransform,
    visible: true, // Node visibility is handled differently in Hyperscape
    add: (child: INode) => node.add(child as unknown as Node),
    remove: (child: INode) => node.remove(child as unknown as Node),
    ...
    getWorldRotation: () => {
      // Strong type assumption - node has matrixWorld
      (node as unknown as THREE.Object3D).matrixWorld.decompose(worldPos, worldQuat, worldScale);
      return worldQuat;
    },
    getWorldScale: () => {
...
export class ClientLoader extends System {
          ...
          const avatar: LoadedAvatar = {
            toNodes(customHooks) {
              ...
              if (customHooks) {
                ...
                if (avatar) {
                  (avatar as unknown as { hooks?: unknown }).hooks = customHooks;
                }
              }
  ...
  insert(type, url, file) {
    ...
    if (type === 'avatar') {
      promise = this.gltfLoader.loadAsync(localUrl).then(glb => {
        const factory = createVRMFactory(glb as unknown as GLBData, this.world.setupMaterial)
        const hooks = this.vrmHooks
        const node = createNode('group', { id: '$root' })
        const node2 = createNode('avatar', { id: 'avatar', factory, hooks })
        node.add(node2)
