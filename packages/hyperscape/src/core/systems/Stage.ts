...
interface MaterialWithFog extends THREE.Material {
  fog: boolean;
}
...
interface MaterialWithShadow extends THREE.Material {
...
function hasEmissiveProperty(material: THREE.Material): material is MaterialWithEmissive {
  return 'emissiveIntensity' in material && typeof (material as unknown as MaterialWithEmissive).emissiveIntensity === 'number';
}
...
export class Stage extends System {
  ...
  override async init(options: unknown): Promise<void> {
    this.viewport = (options as { viewport?: HTMLElement }).viewport;
    if (this.world.rig) {
  ...
  private setMaterialProperty(material: THREE.Material, property: string, value: unknown): void {
    if (material && typeof value !== 'undefined' && hasMaterialProperty(material, property as keyof THREE.Material)) {
      (material as Record<string, unknown>)[property] = value;
    }
  }

  private getMaterialProperty(material: THREE.Material, property: string): unknown {
    return material && hasMaterialProperty(material, property as keyof THREE.Material) 
      ? (material as Record<string, unknown>)[property] 
      : undefined;
  }
  ...
  clean(): void {
    ...
    for (const node of this.dirtyNodes as Set<any>) {
      if (node) {
  ...
  private insertSingle(options: InsertOptions): StageHandle {
    ...
    const sItem: StageItem = {
      ...
      getEntity: () => (node as { ctx?: { entity?: unknown } })?.ctx?.entity,
      node,
    };
  ...
  createMaterial(options: MaterialOptions = {}): MaterialWrapper {
    ...
    return material as unknown as MaterialWrapper;
  }
  ...
  raycastPointer(position: { x: number; y: number }, layers: THREE.Layers = this.maskNone, min = 0, max = Infinity): unknown[] {
    ...
    this.octree.raycast(this.raycaster, this.raycastHits as THREE.Intersection[]);
  ...
  setBackground(background: unknown): void {
    this.scene.background = background as THREE.Color | THREE.Texture | THREE.CubeTexture;
  }

  setFog(fog: unknown): void {
    this.scene.fog = fog as THREE.Fog | THREE.FogExp2;
  }
...
class Model {
  ...
  create(node: unknown, matrix: THREE.Matrix4): StageHandle {
    ...
    const sItem: StageItem = {
      ...
      getEntity: () => (this.items[item.idx]?.node as { ctx?: { entity?: unknown } })?.ctx?.entity,
      node,
    };
  ...
  private getEntity(instanceId: number): unknown {
    ...
    return (this.items[instanceId]?.node as { ctx?: { entity?: unknown } })?.ctx?.entity;
  }
