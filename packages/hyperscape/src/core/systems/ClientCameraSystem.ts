 ...
 * - Third Person/MMORPG (right-click drag, click-to-move)
...
export class ClientCameraSystem extends System {
  ...
  private detectCameraMode(): void {
    // Check if we have RPG systems - if so, default to third person
    if (hasSystem(this.world, 'rpg-player') || hasSystem(this.world, 'rpg-movement') || hasSystem(this.world, 'rpg-entity-manager')) {
      console.log('[ClientCameraSystem] RPG systems detected, switching to third person mode');
      this.setCameraMode({ mode: CameraMode.THIRD_PERSON });
    } else {
      console.log('[ClientCameraSystem] No RPG systems detected, using first person mode');
  ...
  private tryInitialize(): void {
    ...
    // Detect camera mode with longer delay to ensure RPG systems are loaded
    setTimeout(() => {
  ...
  private onCameraModeChanged(oldMode: CameraMode, newMode: CameraMode): void {
    ...
    switch (newMode) {
      case CameraMode.FIRST_PERSON: {
        // Pointer lock disabled for RPG UI
        break;
      }
        
      case CameraMode.THIRD_PERSON: {
        // Pointer lock already disabled for RPG UI
        break;
      }
