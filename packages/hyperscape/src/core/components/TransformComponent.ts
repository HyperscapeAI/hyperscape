...
export class TransformComponent extends Component {
  ...
  get position(): Vector3 {
    const pos = this.get<Vector3 | { x?: number; y?: number; z?: number }>('position');
  ...
  get rotation(): Quaternion {
    const rot = this.get<Quaternion | { x?: number; y?: number; z?: number; w?: number }>('rotation');
  ...
  set rotation(value: Quaternion | { x: number; y: number; z: number; w: number }) {
  ...
  get scale(): Vector3 {
    const scale = this.get<Vector3 | { x?: number; y?: number; z?: number }>('scale');
