...
// Removed unused import: HyperscapeObject3D

import { ActorHandle } from '../../types/index'
...
interface CameraSystem {
  target?: PlayerLocal
  setTarget(player: PlayerLocal): void
  removeTarget(player: PlayerLocal): void
...
// Vector-like interfaces for type safety
...
export class PlayerLocal extends Entity implements HotReloadable {
  ...
  async init(): Promise<void> {
    ...
    // Register with RPG systems and establish integration
    ...
    // Listen for RPG system events to maintain integration
    this.world.on(EventType.PLAYER_HEALTH_UPDATED, this.handleRPGHealthChange.bind(this))
    this.world.on(EventType.PLAYER_TELEPORT_REQUEST, this.handleRPGTeleport.bind(this))
  ...
  initUnifiedSystems(): void {
    ...
    if (movementSystem) {
      ...
      this.world.emit(EventType.PLAYER_REGISTERED, { id: this.data.id })
    }
  ...
  // Handle RPG system integration
  handleRPGHealthChange(event: { playerId: string; health: number; maxHealth: number }): void {
    if (event.playerId === this.data.id) {
      this.nametag!.health = event.health
    }
  }

  handleRPGTeleport(event: {
    playerId: string
