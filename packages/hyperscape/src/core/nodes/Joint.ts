...
// Base PhysX joint interface - all joints have release and break force methods
interface BasePhysXJoint {
  release: () => void;
  setBreakForce: (force: number, torque: number) => void;
  setConstraintFlag?: (flag: number, value: boolean) => void;
}

// Generic joint interface that can handle method calls safely
interface PhysXJoint extends BasePhysXJoint {
  // Optional methods for different joint types
  setLimitCone?: (cone: unknown) => void;
  setSphericalJointFlag?: (flag: number, value: boolean) => void;
  setLimit?: (limit: unknown) => void;
...
interface PhysXController {
  // Controller interface (already defined elsewhere)
}

interface PhysXMoveFlags {
  isSet: (flag: number) => boolean;
}

interface Vector3WithPxTransform extends THREE.Vector3 {
  toPxTransform?: (pose: PhysX.PxTransform) => void;
}

interface QuaternionWithPxTransform extends THREE.Quaternion {
  toPxTransform?: (pose: PhysX.PxTransform) => void;
}
...
interface JointData extends NodeData {
  type?: string;
