/**
 * RPG Unified Logging System
 * Provides consistent logging across all RPG systems with context, formatting, and filtering
 */
...
export const RPGLogger = new RPGLoggerImpl();

// Export logger interface for systems to use
export interface IRPGLogger {
...
export class SystemLogger {
  ...
  debug(message: string, context?: Record<string, unknown>): void {
    RPGLogger.debug(`[${this.systemName}] ${message}`, context);
  }

  info(message: string, context?: Record<string, unknown>): void {
    RPGLogger.system(this.systemName, message, context);
  }

  warn(message: string, context?: Record<string, unknown>): void {
    RPGLogger.systemWarn(this.systemName, message, context);
  }

  error(message: string, error?: Error, context?: Record<string, unknown>): void {
    RPGLogger.systemError(this.systemName, message, error, context);
  }

  player(playerId: string, message: string, context?: Record<string, unknown>): void {
    RPGLogger.player(playerId, `[${this.systemName}] ${message}`, context);
  }

  playerError(playerId: string, message: string, error?: Error, context?: Record<string, unknown>): void {
    RPGLogger.playerError(playerId, `[${this.systemName}] ${message}`, error, context);
  }

  playerWarn(playerId: string, message: string, context?: Record<string, unknown>): void {
    RPGLogger.playerWarn(playerId, `[${this.systemName}] ${message}`, context);
  }
}

// Environment-based configuration
if (typeof process !== 'undefined' && process.env) {
  const logLevel = process.env.RPG_LOG_LEVEL;
  if (logLevel) {
    const level = LogLevel[logLevel as keyof typeof LogLevel];
    if (level !== undefined) {
      RPGLogger.setLogLevel(level);
    }
  }
