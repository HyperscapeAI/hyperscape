...
import type { PlayerData, PlayerHealth, SkillData } from '../types/core';

// Local interface for stats component with proper structure
...
export function executeCombatAttack(
  ...
  targetId: string,
  weaponDamage: number = 0
): { success: boolean; damage?: number; reason?: string } {
...
export function hasEquippedWeapon(player: PlayerData): boolean {
  return !!player.equipment?.weapon
}

export function canUseRanged(player: PlayerData): boolean {
  return (
...
export function getHealthPercentage(player: PlayerData): number {
  return (player.health.current / player.health.max) * 100
}

export function getStaminaPercentage(player: PlayerData): number {
  return (player.stamina.current / player.stamina.max) * 100
}

export function isAlive(player: PlayerData): boolean {
  return player.alive && player.health.current > 0
}

export function isDead(player: PlayerData): boolean {
  return !player.alive || player.health.current <= 0
}