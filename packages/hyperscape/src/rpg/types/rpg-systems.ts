/**
 * RPG system-specific type definitions
 * 
 * This file contains interfaces used by specific RPG systems.
...
import { Equipment, InventoryItem } from './core';
import type { MobStats, RPGItem, Position3D } from './core';
...
export interface LootItem extends RPGItem {
  quantity: number;
}
...
// PlayerData moved to core.ts to avoid duplication
...
export interface StoreSystemInterface {
  ...
  getStoreInventory(storeType: 'general' | 'equipment' | 'food' | 'runes'): Array<{ item: RPGItem; price: number }>;
}
...
export interface WorldPlugin {
  name: string;
  version: string;
  init: ((world: World) => void | Promise<void>) | null;
  systems: Array<new (world: World) => System>;
  dependencies: string[];
