...
export class PersistenceTestSuite {
  ...
  private async testDatabaseOperations(): Promise<void> {
    ...
    try {
      ...
      const testPlayerData: Partial<import('../types/database').RPGPlayerRow> = {
        name: 'Test Player',
        attackLevel: 5,
        attackXp: 100,
        strengthLevel: 3,
  ...
  private async testChunkResetSystem(): Promise<void> {
    ...
    try {
      // Create a test chunk that should be eligible for reset (using detailed RPGWorldChunk interface)
