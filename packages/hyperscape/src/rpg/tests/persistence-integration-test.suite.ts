...
import type { RPGPlayerRow, WorldChunkData } from '../types/database'
import { ItemType, ItemRarity, type PlayerData, type RPGItem } from '../types/index'
import { MockWorld, TestResult } from './test-utils'

// Helper function to create complete RPGItem objects for testing
function createTestItem(overrides: Partial<RPGItem>): RPGItem {
  return {
    id: '1',
    name: 'Test Item',
    type: ItemType.MISC,
...
export class PersistenceIntegrationTestSuite {
  ...
  private async setupTestEnvironment(): Promise<void> {
    try {
      ...
      this.databaseSystem = new RPGDatabaseSystem(this.mockWorld as unknown as World)
      this.mockWorld['rpg-database'] = this.databaseSystem
      await this.databaseSystem.init()

      // Initialize player system
      this.playerSystem = new RPGPlayerSystem(this.mockWorld as unknown as World)
      this.mockWorld['rpg-player-system'] = this.playerSystem
      await this.playerSystem.init()

      // Initialize persistence system
      this.persistenceSystem = new RPGPersistenceSystem(this.mockWorld as unknown as World)
    ...
    try {
      ...
      await Promise.all(
        concurrentPlayers.map(player => {
          const p = player as { playerId: string; playerData: Partial<PlayerData> }
          return this.databaseSystem!.savePlayerData(p.playerId, p.playerData as Partial<RPGPlayerRow>)
        })
      )
      ...
      for (const player of concurrentPlayers) {
        ...
        const loadedPlayer = this.databaseSystem.getPlayerData(p.playerId)
      ...
      for (let i = 0; i < 100; i++) {
        ...
        operations.push(async () => {
          if (this.databaseSystem) {
            ...
            this.databaseSystem.savePlayerData(playerId, playerData as Partial<RPGPlayerRow>)
