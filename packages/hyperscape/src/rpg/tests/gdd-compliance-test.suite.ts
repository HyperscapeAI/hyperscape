...
import { RPGSkillsSystem } from '../systems/RPGSkillsSystem';
...
import { type RPGSkills, type RPGItem } from '../types/index';
import { MobType } from '../types/entities';
import { MockWorld, TestResult } from './test-utils';

// Utility function to create complete mock RPGItem objects
function createMockItem(id: string, name: string, type: ItemType = ItemType.MISC, overrides: Partial<RPGItem> = {}): RPGItem {
  return {
    id,
    name,
    type,
...
export class GDDComplianceTestSuite {
  ...
  private systems: {
    ...
    mobs: RPGMobSystem;
    worldGen: RPGWorldGenerationSystem;
  ...
  private async setupAllSystems(): Promise<void> {
    ...
    try {
      ...
      this.systems.database = new RPGDatabaseSystem(this.mockWorld as unknown as World);
      ...
      this.systems.player = new RPGPlayerSystem(this.mockWorld as unknown as World);
      this.mockWorld['rpg-player-system'] = this.systems.player;
      await this.systems.player.init();
      
      this.systems.combat = new RPGCombatSystem(this.mockWorld as unknown as World);
      this.mockWorld['rpg-combat-system'] = this.systems.combat;
      await this.systems.combat.init();
      
      this.systems.inventory = new RPGInventorySystem(this.mockWorld as unknown as World);
      this.mockWorld['rpg-inventory-system'] = this.systems.inventory;
      await this.systems.inventory.init();
      
      this.systems.skills = new RPGSkillsSystem(this.mockWorld as unknown as World);
      this.mockWorld['rpg-skills-system'] = this.systems.skills;
      await this.systems.skills.init();
      
      this.systems.mobs = new RPGMobSystem(this.mockWorld as unknown as World);
      this.mockWorld['rpg-mob-system'] = this.systems.mobs;
      await this.systems.mobs.init();
      
      this.systems.worldGen = new RPGWorldGenerationSystem(this.mockWorld as unknown as World);
      ...
      this.systems.banking = new RPGBankingSystem(this.mockWorld as unknown as World);
  ...
  private async testGDD_5_1_AvailableSkills(): Promise<void> {
    try {
      ...
      this.mockWorld.emit(EventType.PLAYER_REGISTERED, { id: mockPlayerId });
      ...
      for (const skillName of requiredSkills) {
          const skill = skills[skillName as keyof RPGSkills];
  ...
  private async testGDD_8_1_MobSpawning(): Promise<void> {
    try {
      ...
      const spawnResult = await this.systems.mobs.spawnMob({
        ...
        equipment: {
          weapon: { id: 1, name: 'Goblin Sword', type: AttackType.MELEE },
          armor: { id: 2, name: 'Goblin Armor' }
        },
  ...
  private async testMultiplayerArchitecture(): Promise<void> {
    ...
    try {
      ...
      const playerData = (this.systems.database as unknown as { getPlayerData: (id: string) => unknown }).getPlayerData(testPlayerId);
      if (playerData) {
