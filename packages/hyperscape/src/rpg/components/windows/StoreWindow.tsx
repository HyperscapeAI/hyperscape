...
import { RPGStoreSystem } from '../../systems/RPGStoreSystem'
import { RPGInventorySystem } from '../../systems/RPGInventorySystem'
import { EventType } from '../../../types/events'
import type { RPGStoreItem, InventoryItem } from '../../types/core'
...
export function StoreWindow({ world, visible, onClose, storeId = 'general_store' }: StoreWindowProps) {
  const [storeItems, setStoreItems] = useState<RPGStoreItem[]>([])
  const [playerCoins, setPlayerCoins] = useState(0)
  const [selectedCategory, setSelectedCategory] = useState('all')
  const [selectedItem, setSelectedItem] = useState<RPGStoreItem | null>(null)

  useEffect(() => {
    if (!visible || !world.systems) return

    const storeSystem = world.systems.find(s => 
      s.constructor.name === 'RPGStoreSystem'
    ) as RPGStoreSystem | undefined
    ...
    const updateStoreData = () => {
      ...
      interface StoreSystemInternal {
        stores: Map<string, { items: RPGStoreItem[] }>
      }
      
      if ('stores' in storeSystem) {
        const storeData = (storeSystem as unknown as StoreSystemInternal).stores.get(storeId)
        if (storeData) {
      ...
      // Get player coins using private method access
      interface InventorySystemInternal {
        playerInventories: Map<string, { items: InventoryItem[]; coins: number }>
      }
  ...
  const handleBuyItem = (item: RPGStoreItem, quantity: number = 1) => {
...
function getDefaultStoreItems(): RPGStoreItem[] {
  return [
    // Tools
    {
      id: 'hatchet',
...
function getStoreItemIcon(item: RPGStoreItem): string {
