...
import type { RPGInventorySlotItem } from '../../types/core'
import { EventType } from '../../../types/events'
import type { RPGItem } from '../../data/items'
import type { RPGBankItem } from '../../types/core'
...
export function BankWindow({ world, visible, onClose, bankId = 'bank_town_0' }: BankWindowProps) {
  const [bankItems, setBankItems] = useState<RPGBankItem[]>([])
  const [inventoryItems, setInventoryItems] = useState<RPGInventorySlotItem[]>([])
  const [selectedInventoryItem, setSelectedInventoryItem] = useState<RPGInventorySlotItem | null>(null)
  const [selectedBankItem, setSelectedBankItem] = useState<RPGBankItem | null>(null)
  ...
  useEffect(() => {
    ...
    const bankingSystem = world.systems.find(s => 
      s.constructor.name === 'RPGBankingSystem'
    ) as RPGBankingSystem | undefined
    
    const inventorySystem = world.systems.find(s => 
      s.constructor.name === 'RPGInventorySystem'
    ) as RPGInventorySystem | undefined
    ...
    const updateBankData = () => {
      ...
      interface BankingSystemInternal {
        playerBanks: Map<string, Map<string, { items: RPGBankItem[] }>>
      }
      
      interface InventorySystemInternal {
        playerInventories: Map<string, { items: RPGInventorySlotItem[]; coins: number }>
      }
      
      // Get bank data
      if ('playerBanks' in bankingSystem) {
        const playerBanks = (bankingSystem as unknown as BankingSystemInternal).playerBanks.get(playerId)
        if (playerBanks) {
          const bankData = playerBanks.get(bankId)
          if (bankData) {
      ...
      // Get inventory data
      if ('playerInventories' in inventorySystem) {
        const playerInventory = (inventorySystem as unknown as InventorySystemInternal).playerInventories.get(playerId)
        if (playerInventory) {
  ...
  const handleDepositItem = (inventoryItem: RPGInventorySlotItem) => {
  ...
  const handleWithdrawItem = (bankItem: RPGBankItem, quantity: number = 1) => {
    if (!bankItem) return
...
function getItemIcon(itemData?: { name?: string; type?: string } | RPGItem): string {
  if (!itemData) return '📦'
