...
interface ItemContextMenu {
  ...
  item: RPGInventorySlotItem | null
}

export function InventoryWindow({ world, visible, onClose }: InventoryWindowProps) {
  const [inventory, setInventory] = useState<RPGInventorySlotItem[]>([])
  ...
  const [draggedItem, setDraggedItem] = useState<RPGInventorySlotItem | null>(null)
  const [dragOverSlot, setDragOverSlot] = useState<number | null>(null)

  // Fetch inventory data from RPG systems
  useEffect(() => {
    ...
    if (!inventorySystem) {
      console.warn('[InventoryWindow] No RPGInventorySystem found')
      return
    }
  ...
  const handleItemRightClick = useCallback((event: React.MouseEvent, item: RPGInventorySlotItem) => {
  ...
  const handleItemLeftClick = useCallback((item: RPGInventorySlotItem) => {
  ...
  const getItemActions = (item: RPGInventorySlotItem) => {
    const actions: Array<{
      id: string
      label: string
      icon?: string
  ...
  const handleDragStart = useCallback((event: React.DragEvent<HTMLElement>, item: RPGInventorySlotItem) => {
    setDraggedItem(item)
  ...
  return (
    <>
      ...
      <ContextMenu
        ...
        actions={contextMenu.item ? getItemActions(contextMenu.item) : []}
        onClose={closeContextMenu}
        title={contextMenu.item ? `${contextMenu.item.item?.name || contextMenu.item.itemId}${contextMenu.item.quantity > 1 ? ` (${contextMenu.item.quantity})` : ''}` : undefined}
      />
    </>
  )
}
...
function getItemIcon(itemData?: RPGItem | { name?: string; type?: string } | null): string {
  if (!itemData) return '📦'
