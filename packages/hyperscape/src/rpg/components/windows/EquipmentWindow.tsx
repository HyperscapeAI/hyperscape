...
import { EquipmentSlot, RPGEquipmentSystem } from '../../systems/RPGEquipmentSystem'
...
export function EquipmentWindow({ world, visible, onClose }: EquipmentWindowProps) {
  ...
  // Fetch equipment data from RPG systems
  useEffect(() => {
    if (!visible || !world.systems) return

    const equipmentSystem = world.systems.find(s => 
      s.constructor.name === 'RPGEquipmentSystem'
    ) as RPGEquipmentSystem | undefined
    
    if (!equipmentSystem) {
      console.warn('[EquipmentWindow] No RPGEquipmentSystem found')
      return
    }
    ...
    const updateEquipment = () => {
      // Access equipment data - using type assertion for private property access
      interface EquipmentSystemInternal {
        playerEquipment: Map<string, Record<string, { item: RPGItem | null; itemId: number | null }>>
      }
      
      if ('playerEquipment' in equipmentSystem) {
        const playerEquipmentData = (equipmentSystem as unknown as EquipmentSystemInternal).playerEquipment.get(playerId)
...
// Helper functions
function canEquipItem(item?: RPGItem | null, slot?: string): boolean {
...
function getEquipmentIcon(slot: string, item?: RPGItem | null): string {
  if (!item) return '📦'
