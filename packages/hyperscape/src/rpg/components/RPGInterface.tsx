...
// Using types from core types file
type PlayerStats = RPGPlayerStats & {
...
interface RPGBankData {
  bankId: string
  items: InventoryItem[]
  maxSlots: number
}

interface RPGStoreData {
  storeId: string
  name: string
  items: Array<{
    itemType: string
...
interface RPGUIUpdateEvent {
  playerId: string
  component: 'player' | 'health' | 'inventory' | 'equipment' | 'bank' | 'store'
  data: PlayerStats | { items: InventoryItem[] } | { equipment: LocalEquipment } | (RPGBankData & { isOpen: boolean }) | Record<string, unknown>
}

// Player interface with health property
interface PlayerWithHealth {
  health?: number;
  id?: string;
  name?: string;
  [key: string]: unknown;
...
// Local Equipment interface (renamed to avoid conflict with imported Equipment)
// to include display properties needed by UI
interface LocalEquipmentItem extends InventoryItem {
  name?: string;
  type?: string;
}

interface LocalEquipment {
  [key: string]: LocalEquipmentItem | null;
}
...
export function RPGInterface({ world }: { world: World }) {
  ...
  const [bankData, setBankData] = useState<RPGBankData | null>(null)
  const [storeData, setStoreData] = useState<RPGStoreData | null>(null)
  const [contextMenu, setContextMenu] = useState<RPGContextMenu | null>(null)
  ...
  useEffect(() => {
    ...
    const handleUIUpdate = (data: unknown) => {
      const update = data as RPGUIUpdateEvent
      ...
      switch (update.component) {
        ...
        case 'equipment': {
          const equipmentData = update.data as { equipment: LocalEquipment }
          setEquipment(equipmentData.equipment || equipment)
          break
        }
        case 'bank': {
          const bankData = update.data as RPGBankData & { isOpen: boolean }
          if (bankData.isOpen) {
    ...
    const handleStatsUpdate = (rawData: unknown) => {
      const data = rawData as Partial<PlayerStats> & { playerId: string }
    ...
    const handleEquipmentUpdate = (rawData: unknown) => {
      const data = rawData as { playerId: string; equipment: Equipment }
      if (data.playerId !== localPlayer.id) return
      // Convert Equipment to LocalEquipment format
      const convertedEquipment: LocalEquipment = {}
      if (data.equipment) {
        Object.entries(data.equipment).forEach(([slot, equipmentSlot]) => {
          if (equipmentSlot && typeof equipmentSlot === 'object' && 'id' in equipmentSlot) {
            // Create a temporary InventoryItem-like object with additional properties
            convertedEquipment[slot] = {
              id: equipmentSlot.id,
              itemId: equipmentSlot.id,
              quantity: equipmentSlot.count || 1,
              slot: 0, // Equipment slot index not needed here
              // Store additional properties for UI display
              name: equipmentSlot.name,
            } as InventoryItem & { name: string }
          }
        })
      }
    ...
    const handleBankOpen = (rawData: unknown) => {
      const data = rawData as RPGBankData & { playerId: string }
      if (data.playerId !== localPlayer.id) return
      setBankData(data)
      setShowBank(true)
    }
    ...
    const handleStoreOpen = (rawData: unknown) => {
      const data = rawData as RPGStoreData & { playerId: string }
      if (data.playerId !== localPlayer.id) return
      setStoreData(data)
      setShowStore(true)
    }
    ...
    const handleContextMenu = (rawData: unknown) => {
      const data = rawData as RPGContextMenu & { playerId: string }
      if (data.playerId !== localPlayer.id) return
      setContextMenu(data)
    }
    ...
    const interval = setInterval(() => {
      ...
      if (player && player.health !== undefined) {
        setPlayerStats(prev => {
          if (!prev) {
            return {
              id: player.id || 'player', // Add missing id property
              name: player.name || 'Player', // Add missing name property
              health: {
              ...
              maxStamina: 100,
              level: 1,
              combatLevel: 3, // Add missing combatLevel property
              xp: 0,
              maxXp: 83,
              coins: 0,
              inCombat: false, // Add missing inCombat property
          ...
          return {
            ...prev,
            health: {
              current: Math.floor((player as unknown as PlayerWithHealth).health ?? 100),
              max: prev.health?.max || 100
            },
            stamina: prev.stamina || 100,
            maxStamina: prev.maxStamina || 100,
            combatLevel: prev.combatLevel || 3, // Ensure combatLevel is included
            inCombat: prev.inCombat || false, // Ensure inCombat is included
          }
        })
      }
    }, 100)
  ...
  return (
    <>
      ...
      <RPGHotbar stats={playerStats} inventory={inventory} world={world} />
...
// Bank Component  
function RPGBank({ data, onClose, world }: { data: RPGBankData; onClose: () => void; world: World }) {
  const { bankId: _bankId, items, maxSlots } = data
  const bankName = 'Bank'
      ...
      >
        {slots.map((item, index) => (
          ...
          >
            {item && (
              <>
                <div>{getItemDisplayProps(item).name.substring(0, 8)}</div>
                {item.quantity > 1 && (
                  <div
                    style={{
                      position: 'absolute',
...
// Store Component
function RPGStorePanel({ data, onClose, world }: { data: RPGStoreData; onClose: () => void; world: World }) {
  const { storeId, name, items } = data
    ...
    >
      ...
      >
        {items.map((item, index) => (
          ...
          >
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 'bold' }}>{item.itemType}</div>
            </div>
            <div style={{ textAlign: 'right' }}>
              <div style={{ color: '#fbbf24', fontWeight: 'bold' }}>{item.price} gp</div>
              <div style={{ fontSize: '0.875rem', color: item.stock > 0 ? '#4ade80' : '#ef4444' }}>
                Stock: {item.stock}
              </div>
            </div>
            <button
              onClick={() => handleBuy(item.itemType)}
              disabled={item.stock === 0}
              style={{
                background: item.stock === 0 ? '#4b5563' : '#10b981',
                border: 'none',
                borderRadius: '0.25rem',
                color: 'white',
                padding: '0.5rem 1rem',
...
function RPGHotbar({ stats: _stats, inventory, world }: { stats: PlayerStats | null; inventory: InventoryItem[]; world: World }) {
