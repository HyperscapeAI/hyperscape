...
export class EquipmentBonusCalculator {
   ...
   */
  calculateTotalBonuses(equipment: Record<string, RPGItem | null>): CombatBonuses {
   ...
   */
  meetsRequirements(item: RPGItem, stats: StatsComponent): boolean {
   ...
   */
  getEquipmentWeight(equipment: Record<string, RPGItem | null>): number {
    let totalWeight = 0;
   ...
   */
  getSetBonuses(equipment: Record<string, RPGItem | null>): CombatBonuses {
    ...
    const equippedItems = Object.values(equipment).filter(item => item !== null) as RPGItem[];
   ...
   */
  private hasCompleteSet(items: RPGItem[], setName: string): boolean {
   ...
   */
  private hasVoidSet(items: RPGItem[]): boolean {
   ...
   */
  calculateWeightReduction(equipment: Record<string, RPGItem | null>): number {
    let reduction = 0;
   ...
   */
  getPrayerDrainReduction(equipment: Record<string, RPGItem | null>): number {
    let reduction = 0;
