...
export class ItemRegistry {
  private items: Map<number, RPGItem> = new Map();
  private nameIndex: Map<string, RPGItem> = new Map();

  /**
   * Register an item definition
   */
  register(item: RPGItem): void {
   ...
   * Get item by ID
   */
  get(itemId: number): RPGItem | null {
   ...
   */
  getByName(name: string): RPGItem | null {
   ...
   */
  getByCategory(category: string): RPGItem[] {
    const results: RPGItem[] = [];
