...
import { PlayerSpawnSystemInfo as SystemInfo } from '../types/rpg-system-types';
import { RPGSystemBase } from './RPGSystemBase';

export class RPGPlayerSpawnSystem extends RPGSystemBase {
  ...
  private async spawnPlayerWithEquipment(playerId: string): Promise<void> {
    const spawnData = this.spawnedPlayers.get(playerId)!;
      ...
      this.emitTypedEvent(EventType.PLAYER_REGISTERED, { id: playerId });
