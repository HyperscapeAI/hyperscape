...
import type { RPGNPCSystem } from '../../systems/RPGNPCSystem'
import { EventType } from '../../../types/events'

// Mock RPGNPCSystem interface
interface MockRPGNPCSystemInterface {
  spawnNPC(config: unknown): unknown
}

// Mock RPGNPCSystem
class MockRPGNPCSystem implements MockRPGNPCSystemInterface {
  spawnNPC = vi.fn()
  constructor() {}
}
...
describe('NPCSpawnManager', () => {
  ...
  let mockWorld: World
  let mockNPCSystem: MockRPGNPCSystem

  beforeEach(() => {
    vi.useFakeTimers()
    mockWorld = createMockWorld()
    mockNPCSystem = new MockRPGNPCSystem()
    
    // Mock console.log to avoid noise in tests
    vi.spyOn(console, 'log').mockImplementation(() => {})
    
    // Create spawn manager with skipDefaults option for test isolation
    spawnManager = new NPCSpawnManager(mockWorld, mockNPCSystem as unknown as RPGNPCSystem, { skipDefaults: true })
  ...
  describe('constructor', () => {
    ...
    it('should register default spawn points from world areas when skipDefaults is false', () => {
      ...
      const managerWithDefaults = new NPCSpawnManager(mockWorld, mockNPCSystem as unknown as RPGNPCSystem, { skipDefaults: false })
    ...
    it('should log spawn point count on initialization with defaults', () => {
      ...
      // Create a new spawn manager without skipping defaults
      new NPCSpawnManager(mockWorld, mockNPCSystem as unknown as RPGNPCSystem, { skipDefaults: false })
  ...
  describe('integration with world events', () => {
    ...
    it('should handle world without events system', () => {
      ...
      const mockNPCSystemWithSpawn = new MockRPGNPCSystem();
      mockNPCSystemWithSpawn.spawnNPC.mockReturnValue({ id: 'test-npc' });
    
      expect(() => {
        const manager = new NPCSpawnManager(worldWithoutEvents, mockNPCSystemWithSpawn as unknown as RPGNPCSystem, { skipDefaults: true });
