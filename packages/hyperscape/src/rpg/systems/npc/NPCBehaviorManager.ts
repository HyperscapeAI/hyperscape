...
export class NPCBehaviorManager {
  ...
  private updateMovement(npc: NPCEntity, npcComponent: NPCComponent): void {
    ...
    this.world.getSystem<RPGMovementSystem>('rpg-movement')?.moveEntity(npc.id, movement.targetPosition);
   ...
   */
  private faceEntity(npc: NPCEntity, target: Entity | RPGEntity): void {
    const npcPos = this.getEntityPosition(npc);
  ...
  private canAttackPlayer(npc: NPCEntity, player: PlayerEntity): boolean {
    ...
    if (stats?.health !== undefined && stats.health <= 0) return false;
   ...
   * Get entity position
   */
  private getEntityPosition(entity: Entity | RPGEntity): Position3D | null {
    if ('getPosition' in entity && typeof entity.getPosition === 'function') {
      return (entity as RPGEntity).getPosition();
    }
    if (entity.position) {
      return {
        x: entity.position.x,
   ...
   */
  private spatialQuery(position: Position3D, range: number): (Entity | RPGEntity)[] {
    const results: (Entity | RPGEntity)[] = [];
    ...
    for (const entity of allEntities) {
      ...
      if (entityPos) {
        ...
        if (distance <= range) {
          results.push(entity as Entity | RPGEntity);
        }
      }
    }
