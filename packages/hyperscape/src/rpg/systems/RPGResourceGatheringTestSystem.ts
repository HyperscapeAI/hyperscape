 ...
 * - Tests unified resource gathering through RPGResourceSystem
 * - Tests processing through RPGProcessingSystem  
 * - Tests skill progression through RPGXPSystem
...
export class RPGResourceGatheringTestSystem extends RPGVisualTestFramework {
  ...
  private processingSystem!: RPGProcessingSystem;
  ...
  async init(): Promise<void> {
    ...
    this.resourceSystem = getSystem<RPGResourceSystem>(this.world, 'rpg-resource')!;
    this.processingSystem = getSystem<RPGProcessingSystem>(this.world, 'rpg-processing')!;
    this.xpSystem = getSystem<RPGSkillsSystem>(this.world, 'rpg-skills')!;
  ...
  private runComprehensiveSequence(stationId: string): void {
    ...
    const nextPhase = () => {
      ...
      const currentPhase = phases[phase];
              RPGLogger.system('RPGResourceGatheringTestSystem', `[${stationId}] Starting phase: ${currentPhase}`);
  ...
  private validateStationPosition(stationId: string): Position3D | null {
    const station = this.testStations.get(stationId);
    if (!station) {
      RPGLogger.systemError('RPGResourceGatheringTestSystem', `Station not found: ${stationId}`);
