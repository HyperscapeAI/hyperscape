...
import type { Player, MovementTestData } from '../types/test';
...
// Player interface for movement testing with visual proxy
interface PlayerWithProxy extends Player {
...
export class RPGMovementTestSystem extends RPGVisualTestFramework {
  ...
  private movementSystem!: RPGMovementSystem;
  private pathfindingSystem!: RPGPathfindingSystem;

  constructor(world: World) {
    super(world);
    const ms = world.getSystem<RPGMovementSystem>('rpg-movement')
    if(!ms) {
      throw new Error("No movement system found");
    }
    this.movementSystem = ms;

    const pf = world.getSystem<RPGPathfindingSystem>('rpg-pathfinding')
    if(!pf) {
    ...
    const pathfindToNextWaypoint = async () => {
      ...
      if (this.pathfindingSystem && this.movementSystem && currentTestData.player) {
        ...
        try {
          const playerExists = this.world.entities.get(currentTestData.player.id);
          if (!playerExists) {
            // console.log(`[RPGMovementTestSystem] Player ${currentTestData.player.id} no longer exists, stopping pathfinding test`);
            return;
          }
        } catch (_error) {
          // console.log(`[RPGMovementTestSystem] Error checking player existence: ${_error}`);
          return;
        }
    ...
    const testMovementAtWaypoint = async () => {
      ...
      if (this.movementSystem && currentTestData.player) {
        ...
        try {
          const playerExists = this.world.entities.get(currentTestData.player.id);
          if (!playerExists) {
            // console.log(`[RPGMovementTestSystem] Player ${currentTestData.player.id} no longer exists, stopping collision test`);
            return;
          }
        } catch (_error) {
          // console.log(`[RPGMovementTestSystem] Error checking player existence: ${_error}`);
    ...
    const teleportToNextTarget = async () => {
      ...
      if (this.movementSystem && currentTestData.player) {
        ...
        try {
          const playerExists = this.world.entities.get(currentTestData.player.id);
          if (!playerExists) {
            // console.log(`[RPGMovementTestSystem] Player ${currentTestData.player.id} no longer exists, stopping teleportation test`);
            return;
          }
        } catch (_error) {
          // console.log(`[RPGMovementTestSystem] Error checking player existence: ${_error}`);
        ...
        try {
          ...
          // console.warn('[RPGMovementTestSystem] teleportPlayer warning:', _error);
        }
      } else {
        // Fallback: Use movePlayer instead of teleportPlayer
        // console.warn('[RPGMovementTestSystem] teleportPlayer not available, using movePlayer as fallback');
  ...
  private startComprehensiveSequence(stationId: string): void {
    ...
    // console.log('[RPGMovementTestSystem] Starting comprehensive movement test sequence');
    ...
    const phase2TimeoutId = setTimeout(() => {
      const currentTestData = this.testData.get(stationId);
      if (currentTestData) {
        // console.log('[RPGMovementTestSystem] Phase 2: Pathfinding');
    ...
    const phase3TimeoutId = setTimeout(() => {
      const currentTestData = this.testData.get(stationId);
      if (currentTestData) {
        // console.log('[RPGMovementTestSystem] Phase 3: Mixed movement');
    ...
    const completeTimeoutId = setTimeout(() => {
      // console.log('[RPGMovementTestSystem] Completing comprehensive test');
  ...
  protected cleanupTest(stationId: string): void {
    ...
    if (testData) {
      ...
      const playerWithProxy = testData.player as PlayerWithProxy;
  ...
  private completeBasicMovementTest(stationId: string): void {
    ...
    // console.log(`[RPGMovementTestSystem] Basic movement test ${success ? 'PASSED' : 'FAILED'} for station ${stationId}${error ? ': ' + error : ''}`);
  ...
  private completePathfindingTest(stationId: string): void {
    ...
    // console.log(`[RPGMovementTestSystem] Pathfinding test ${success ? 'PASSED' : 'FAILED'} for station ${stationId}${error ? ': ' + error : ''}`);
    this.updateStationStatus(stationId, success ? 'passed' : 'failed', error);
  }
  ...
  private completeCollisionTest(stationId: string): void {
    ...
    // console.log(`[RPGMovementTestSystem] Collision test ${success ? 'PASSED' : 'FAILED'} for station ${stationId}${error ? ': ' + error : ''}`);
    this.updateStationStatus(stationId, success ? 'passed' : 'failed', error);
  }
  ...
  private completeTeleportationTest(stationId: string): void {
    ...
    // console.log(`[RPGMovementTestSystem] Teleportation test ${success ? 'PASSED' : 'FAILED'} for station ${stationId}${error ? ': ' + error : ''}`);
    this.updateStationStatus(stationId, success ? 'passed' : 'failed', error);
  }
  ...
  private completeComprehensiveTest(stationId: string): void {
    ...
    // console.log(`[RPGMovementTestSystem] Comprehensive test ${success ? 'PASSED' : 'FAILED'} for station ${stationId}${error ? ': ' + error : ''}`);
    this.updateStationStatus(stationId, success ? 'passed' : 'failed', error);
  }
  ...
  protected updateStationStatus(stationId: string, status: 'idle' | 'running' | 'passed' | 'failed', error?: string): void {
    ...
    if (station) {
      ...
      // console.log(`[RPGMovementTestSystem] Updated station ${stationId} status to ${status}${error ? ': ' + error : ''}`);
    }
  }
}