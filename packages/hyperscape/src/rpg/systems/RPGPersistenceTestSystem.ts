...
import { Vector3 } from '../../core/extras/three';
import { getItem, RPGItem } from '../data/items';
import { RPGSkills, World, InventoryItem } from '../types/core';
...
interface PersistenceTestData {
  ...
  originalData: {
    ...
    equipment: Array<{ slot: string; item: RPGItem }>;
...
export class RPGPersistenceTestSystem extends RPGVisualTestFramework {
  ...
  private rpgSkillsSystem!: RPGSkillsSystem;
  ...
  async init(): Promise<void> {
    ...
    this.persistenceSystem = getSystem<RPGPersistenceSystem>(this.world, 'rpg-persistence')!;
    this.inventorySystem = getSystem<RPGInventorySystem>(this.world, 'rpg-inventory')!;
    this.rpgSkillsSystem = getSystem<RPGSkillsSystem>(this.world, 'rpg-skills')!;
