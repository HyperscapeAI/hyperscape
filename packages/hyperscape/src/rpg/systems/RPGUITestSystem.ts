...
import type { RPGUISystem } from './RPGUISystem';
...
export class RPGUITestSystem extends RPGVisualTestFramework {
  ...
  async init(): Promise<void> {
    ...
    this.uiSystem = getSystem<RPGUISystem>(this.world, 'rpg-ui')!;
    this.inventorySystem = getSystem<RPGInventorySystem>(this.world, 'rpg-inventory')!;
    this.equipmentSystem = getSystem<RPGEquipmentSystem>(this.world, 'rpg-equipment')!;
    this.bankingSystem = getSystem<RPGBankingSystem>(this.world, 'rpg-banking')!;
  ...
  private async runEquipmentUITest(stationId: string): Promise<void> {
    const station = this.testStations.get(stationId)!;
      ...
      const player = createdPlayer as unknown as Player;
  ...
  private async runHealthStaminaUITest(stationId: string): Promise<void> {
    try {
      ...
      // Cast to Player type and ensure proper structure
      const player = createdPlayer as unknown as Player;
      // Set health using proper PlayerHealth type
      player.health = { current: 65, max: 100 }; // Damaged health to test display
      player.inventory = { items: [], capacity: 28, coins: 0 };
      player.equipment = {
        weapon: null,
        shield: null,
