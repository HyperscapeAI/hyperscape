...
export class RPGDatabaseTestSystem extends RPGVisualTestFramework {
  ...
  async init(): Promise<void> {
    ...
    const system = this.world.getSystem('rpg-database');
    if (!system) {
      throw new Error('[RPGDatabaseTestSystem] RPGDatabaseSystem not found');
    }
    this.databaseSystem = system as RPGDatabaseSystem;
    ...
    const testConfigs = [
      {
        ...
        testType: 'crud_operations' as const
      },
      {
        id: 'inventory-persistence-test',
        name: 'Inventory Persistence Test',
        position: { x: 20, y: 1, z: 20 },
        testType: 'inventory_persistence' as const
      },
      {
        id: 'equipment-storage-test',
        name: 'Equipment Storage Test',
        position: { x: 30, y: 1, z: 20 },
        testType: 'equipment_storage' as const
      },
      {
        id: 'chunk-management-test',
        name: 'Chunk Management Test',
        position: { x: 40, y: 1, z: 20 },
        testType: 'chunk_management' as const
      },
      {
        id: 'session-tracking-test',
        name: 'Session Tracking Test',
        position: { x: 50, y: 1, z: 20 },
        testType: 'session_tracking' as const
      },
      {
        id: 'transactions-test',
        name: 'Transactions Test',
        position: { x: 60, y: 1, z: 20 },
        testType: 'transactions' as const
      },
      {
        id: 'comprehensive-database-test',
        name: 'Comprehensive Database Test',
  ...
  private async runCRUDOperationsTest(stationId: string): Promise<void> {
    ...
    const testPlayerId = 'test-crud-player';
      ...
      const createData: Partial<RPGPlayerRow> = {
      ...
      this.databaseSystem.savePlayerData(testPlayerId, createData);
      testData.operationsPerformed['create'] = 1;
      testData.dataCreated[testPlayerId] = true;

      // READ - Test player retrieval
      const retrievedData = this.databaseSystem.getPlayerData(testPlayerId);
      if (retrievedData) {
      ...
      const updateData: Partial<RPGPlayerRow> = {
        combatLevel: 5,
        attackXp: 1000,
        coins: 500,
        health: 80
      ...
      this.databaseSystem.savePlayerData(testPlayerId, updateData);
      testData.operationsPerformed['update'] = 1;

      // Verify update
      const updatedData = this.databaseSystem.getPlayerData(testPlayerId);
      ...
      for (let i = 0; i < 10; i++) {
        this.databaseSystem.savePlayerData(`bulk-test-${i}`, {
  ...
  private async runInventoryPersistenceTest(stationId: string): Promise<void> {
    ...
    const testPlayerId = 'test-inventory-player';
      // Create player first
      this.databaseSystem.savePlayerData(testPlayerId, {
        playerId: testPlayerId,
  ...
  private async runTransactionsTest(stationId: string): Promise<void> {
    const testData = this.testData.get(stationId)!;
      ...
      const player = this.databaseSystem.getPlayerData(testPlayerId);
      ...
      try {
        this.databaseSystem.savePlayerData('', { 
          playerId: '',
          name: 'Invalid Player'
        });
      } catch (error) {
