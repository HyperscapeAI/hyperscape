...
import { RPGStore } from '../types/core';
import type {
  RPGStoreBuyEvent,
  RPGStoreCloseEvent,
  RPGStoreSellEvent,
 ...
 */
export class RPGStoreSystem extends RPGSystemBase {
  private stores = new Map<StoreID, RPGStore>();
  ...
  async init(): Promise<void> {
    ...
    this.subscribe<RPGStoreOpenEvent>(EventType.STORE_OPEN, (event) => {
      this.openStore(event.data);
    });
    this.subscribe<RPGStoreCloseEvent>(EventType.STORE_CLOSE, (event) => {
      this.closeStore(event.data);
    });
    this.subscribe<RPGStoreBuyEvent>(EventType.STORE_BUY, (event) => {
      this.buyItem(event.data);
    });
    this.subscribe<RPGStoreSellEvent>(EventType.STORE_SELL, (event) => {
  ...
  private openStore(data: RPGStoreOpenEvent): void {
    const storeId = createStoreID(data.storeId);
  ...
  private closeStore(data: RPGStoreCloseEvent): void {
  ...
  private buyItem(data: RPGStoreBuyEvent): void {
    const storeId = createStoreID(data.storeId);
    const store = this.stores.get(storeId)!; // Store must exist
  ...
  public sellItem(playerId: string, itemId: string, quantity: number, _expectedPrice?: number): boolean {
    ...
    let targetStore: RPGStore | undefined;
  ...
  public getAllStores(): RPGStore[] {
    return Array.from(this.stores.values());
  }

  public getStore(storeId: string): RPGStore | undefined {
