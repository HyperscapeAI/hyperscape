...
export class DropCalculator {
  ...
  calculateDrops(lootTable: LootTable, _killer?: RPGEntity | null): LootDrop[] {
    ...
    if (regularDrops.length > 0) {
      // Convert RPGItem[] to LootDrop[]
      const lootDrops: LootDrop[] = regularDrops.map(item => ({
        itemId: item.id,
        quantity: 1, // RPGItem doesn't have quantity, default to 1
        weight: undefined, // RPGItem doesn't have weight
        rarity: item.rarity as 'common' | 'uncommon' | 'rare' | 'very_rare' | undefined,
        rare: false // RPGItem doesn't have rare flag
      }));
