...
export class RPGPhysicsTestSystem extends RPGSystemBase {
  ...
  private async testBasicRaycasting(): Promise<void> {
    ...
    if (hit && hit.point) {
      ...
      RPGLogger.system('RPGPhysicsTestSystem', 'Basic raycast hit detected at', { point: hit.point });
    } else {
  ...
  private async testCombatRangeDetection(): Promise<void> {
    ...
    this.testResults.set('ranged_range', rangedInRange);
          ...
          RPGLogger.system('RPGPhysicsTestSystem', `Combat range test: melee=${meleeDistance.toFixed(2)}m (${meleeInRange}), ranged=${rangedDistance.toFixed(2)}m (${rangedInRange})`);
  }
  ...
  private reportResults(): void {
    RPGLogger.system('RPGPhysicsTestSystem', 'Test Results:');
    for (const [test, passed] of this.testResults) {
      RPGLogger.system('RPGPhysicsTestSystem', `  ${test}: ${passed ? '✅ PASS' : '❌ FAIL'}`);
    }
  }
}
