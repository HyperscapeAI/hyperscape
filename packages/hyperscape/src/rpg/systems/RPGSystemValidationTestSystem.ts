...
export class RPGSystemValidationTestSystem extends RPGSystemBase {
  ...
  constructor(world: World) {
    super(world, { 
      name: 'rpg-system-validation-test',
      dependencies: {
        required: ['rpg-equipment', 'rpg-item-pickup', 'rpg-item-actions', 'rpg-loot', 'rpg-player', 'rpg-combat', 'rpg-inventory', 'rpg-mob'],
        optional: []
      },
      autoCleanup: true
    });
  ...
  private completeValidationTest(results: SystemValidationResult[]): void {
    ...
    this.emitTypedEvent('rpg:test:update_cube', {
      id: 'system_validation_indicator',
  ...
  private createValidationReport(results: ValidationResults): void {
    ...
    this.emitTypedEvent('rpg:test:validation:completed', {
      results,
