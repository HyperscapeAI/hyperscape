...
export class RPGPlayerTestSystem extends RPGVisualTestFramework {
  ...
  async init(): Promise<void> {
    ...
    this.playerSystem = this.world.getSystem<RPGPlayerSystem>('rpg-player')!;
    this.databaseSystem = this.world.getSystem<RPGDatabaseSystem>('rpg-database')!;
  ...
  protected runTest(stationId: string): void {
    ...
    if (!testData) {
      console.error(`[RPGPlayerTestSystem] No test data found for station: ${stationId}`);
      console.error(`[RPGPlayerTestSystem] Available stations:`, Array.from(this.testData.keys()));
  ...
  private async runPersistenceTest(stationId: string): Promise<void> {
    ...
    try {
      ...
      // Save player data
      await this.savePlayerData(testPlayerId, {
      ...
      // Load player data  
      const loadedData = await this.loadPlayerData(testPlayerId);
      if (loadedData) {
  ...
  private async runMultiPlayerTest(stationId: string): Promise<void> {
    ...
    for (let i = 0; i < testPlayerIds.length - 1; i++) {
      ...
      const isolated = await this.testPlayerDataIsolation(player1Id, player2Id);
    ...
    const concurrentOperations = testPlayerIds.map(async playerId => {
      ...
      return this.savePlayerData(playerId, { health: Math.floor(Math.random() * 100) });
    });
  ...
  private async registerTestPlayer(playerId: string, playerData: Partial<RPGPlayerRow>): Promise<void> {
  ...
  private async savePlayerData(playerId: string, data: Partial<RPGPlayerRow>): Promise<void> {
    // Save player data via database system
    this.databaseSystem.savePlayerData(playerId, data);
  }

  private async loadPlayerData(playerId: string): Promise<RPGPlayerRow | undefined> {
    // Load player data via database system
    return this.databaseSystem.getPlayerData(playerId) || undefined;
  }

  private async testPlayerDataIsolation(player1Id: string, player2Id: string): Promise<boolean> {
