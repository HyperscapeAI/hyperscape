...
import type { PhysXModule } from '../../types/physx.d';
 ...
 */
export class RPGPrecisionPhysicsTestSystem extends RPGSystemBase {
  ...
  async init(): Promise<void> {
    RPGLogger.system('PrecisionPhysics', 'System initialized');
    ...
    this.world.on('precision_physics_test', (testType: string) => {
      RPGLogger.system('PrecisionPhysics', `Received test request: ${testType}`)
      this.runSpecificTest(testType);
    });
  }

  start(): void {
          RPGLogger.system('PrecisionPhysics', 'Starting precision physics tests')
    ...
    RPGLogger.system('PrecisionPhysics', 'All test scenarios created')
  ...
  private getPhysX(): PhysXModule {
    return (globalThis as unknown as { PHYSX: PhysXModule }).PHYSX;
  }

  private createGroundPlane(): void {
    const PHYSX = this.getPhysX();
    RPGLogger.system('PrecisionPhysics', 'Creating ground plane with PHYSX');
    ...
    this.world.physics.addActor(groundActor, {
      ...
      triggeredHandles: new Set()
          ...
          RPGLogger.system('PrecisionPhysics', 'Ground plane created successfully');
  ...
  private createProjectileMotionTest(): void {
          RPGLogger.system('PrecisionPhysics', 'Creating projectile motion test')
    ...
    projectile.userData = {
      ...
      if (actor.getActorFlags && PHYSX.PxActorFlagEnum) {
        ...
        if (typeof actorFlags === 'number' && (actorFlags & PHYSX.PxActorFlagEnum.eDISABLE_GRAVITY)) {
          RPGLogger.system('PrecisionPhysics', 'Gravity was disabled, enabling...');
      ...
      actor.setSleepThreshold(0); // Disable sleeping
              ...
              RPGLogger.system('PrecisionPhysics', 'Actor created and added to scene');
      ...
      this.precisionTests.set('projectile_motion', {
        ...
        calculatedValues: {
          ...
          expectedLandingPosition: expectedLandingPosition
              ...
              RPGLogger.system('PrecisionPhysics', 'Projectile motion test created successfully');
  }
  ...
  private testProjectileMotion(): void {
          RPGLogger.system('PrecisionPhysics', 'Starting projectile motion test')
    ...
    RPGLogger.system('PrecisionPhysics', `Launch velocity: (${launchVelocity.x}, ${launchVelocity.y}, ${launchVelocity.z})`);
    const launchPosition = projectile.userData.launchPosition as THREE.Vector3;
    RPGLogger.system('PrecisionPhysics', `Launch position: (${launchPosition.x}, ${launchPosition.y}, ${launchPosition.z})`);
  ...
  private validateProjectileMotionTest(): void {
    RPGLogger.system('PrecisionPhysics', 'Validating projectile motion test');
    const test = this.precisionTests.get('projectile_motion');
    if (!test) return;
    ...
    if (physicsActor) {
      ...
      RPGLogger.system('PrecisionPhysics', `Actor physics position: (${pose.p.x.toFixed(2)}, ${pose.p.y.toFixed(2)}, ${pose.p.z.toFixed(2)})`);
    } else {
      finalPosition = projectile.position;
      RPGLogger.system('PrecisionPhysics', `Using mesh position: (${finalPosition.x.toFixed(2)}, ${finalPosition.y.toFixed(2)}, ${finalPosition.z.toFixed(2)})`);
    }
    ...
    RPGLogger.system('PrecisionPhysics', `Expected position: (${expectedPosition.x.toFixed(2)}, ${expectedPosition.y.toFixed(2)}, ${expectedPosition.z.toFixed(2)})`);
    ...
    RPGLogger.system('PrecisionPhysics', `Launch position was: (${launchPos.x}, ${launchPos.y}, ${launchPos.z})`);
    RPGLogger.system('PrecisionPhysics', `Launch velocity was: (${launchVel.x}, ${launchVel.y}, ${launchVel.z})`);
  ...
  private testFrictionCoefficient(): void {
          RPGLogger.system('PrecisionPhysics', 'Starting friction coefficient test')
  ...
  private testAngularMomentum(): void {
          RPGLogger.system('PrecisionPhysics', 'Starting angular momentum test')
  ...
  private runAllPrecisionTests(): void {
    RPGLogger.system('PrecisionPhysics', 'Running all precision tests')
  ...
  private runSpecificTest(testType: string): void {
    switch (testType) {
      ...
      case 'all':
        ...
        break;
      default:
        RPGLogger.systemWarn('PrecisionPhysics', `Unknown test type: ${testType}`);
    }
  }
    ...
    if (projectileTest && projectileTest.testStarted && !projectileTest.testCompleted) {
      ...
      if (physicsActor) {
        ...
        if (this.updateCounter % 10 === 0) {
          RPGLogger.system('PrecisionPhysics', `Projectile update - Pos: (${pose.p.x.toFixed(2)}, ${pose.p.y.toFixed(2)}, ${pose.p.z.toFixed(2)})` +
    ...
    if (projectileTest && projectileTest.testStarted && !projectileTest.testCompleted) {
      ...
      if (physicsActor) {
        ...
        if (pose.p.y <= 0.15) { // radius of projectile
          RPGLogger.system('PrecisionPhysics', `Projectile hit ground at position: (${pose.p.x.toFixed(2)}, ${pose.p.y.toFixed(2)}, ${pose.p.z.toFixed(2)})`);
