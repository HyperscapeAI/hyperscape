...
import { EquipmentSlotName, PlayerEquipment, InventoryItem, RPGItem } from '../types/core'
...
export class RPGEquipmentTestSystem extends RPGVisualTestFramework {
  ...
  private createInventoryItem(item: RPGItem, quantity: number, slot: number = 0): InventoryItem {
    return {
  ...
  private getPlayerStatsAsRecord(playerId: string): Record<string, number> {
    ...
    return {
      attack: (statsComponent.attack as any)?.level || 0,
      strength: (statsComponent.strength as any)?.level || 0,
      defense: (statsComponent.defense as any)?.level || 0,
      ranged: (statsComponent.ranged as any)?.level || 0,
      constitution: (statsComponent.constitution as any)?.level || 0
    }
  }
  ...
  async init(): Promise<void> {
    ...
    const equipmentSystem = getSystem<RPGEquipmentSystem>(this.world, 'rpg-equipment')
    if (!equipmentSystem) {
      throw new Error('[RPGEquipmentTestSystem] EquipmentSystem is required')
    }
    this.equipmentSystem = equipmentSystem
    
    const xpSystem = getSystem<RPGSkillsSystem>(this.world, 'rpg-skills')
    if (!xpSystem) {
      throw new Error('[RPGEquipmentTestSystem] XPSystem is required')
    }
  ...
  private async runBasicEquipmentTest(stationId: string): Promise<void> {
    try {
      ...
      this.testData.set(stationId, {
        ...
        testItems,
        equipmentSlotsBefore: player.equipment,
        equipmentSlotsAfter: { weapon: null, shield: null, helmet: null, body: null, legs: null, arrows: null },
      })
  ...
  private startStatChangesSequence(stationId: string): void {
    ...
    const equipAndCheckStats = async () => {
      ...
      if (!statsComponent) {
        RPGLogger.systemWarn('RPGEquipmentTestSystem', 'Could not get stats for player', { playerId: testData.player.id })
        return
      }
      ...
      if (this.equipmentSystem) {
        ...
        setTimeout(async () => {
          ...
          if (!statsComponentAfter) {
            RPGLogger.systemWarn('RPGEquipmentTestSystem', 'Could not get stats after equipping', { playerId: testData.player.id })
            return
          }
          const statsAfter: Record<string, number> = {
            attack: (statsComponentAfter.attack as any)?.level || 0,
            strength: (statsComponentAfter.strength as any)?.level || 0,
            defense: (statsComponentAfter.defense as any)?.level || 0,
            ranged: (statsComponentAfter.ranged as any)?.level || 0,
            constitution: (statsComponentAfter.constitution as any)?.level || 0
          }
  ...
  private startLevelRequirementsSequence(stationId: string): void {
    ...
    const equipmentSuccessListener = (data: { playerId: string; item: { name: string } }) => {
      if (data.playerId === testData.player.id) {
        ...
        RPGLogger.system('RPGEquipmentTestSystem', `Equipment success: ${data.item.name} for ${data.playerId}`)
      }
    }
    ...
    const equipmentFailureListener = (data: { playerId: string; itemId: string; error: string }) => {
      if (data.playerId === testData.player.id) {
        pendingEquipmentAttempts--
        RPGLogger.system('RPGEquipmentTestSystem', `Equipment failure: ${data.itemId} for ${data.playerId} - ${data.error}`)
      }
    }
