...
import { getNearestTown, WORLD_CONSTANTS, type DeathLocationData } from '../data/world-structure';
import type { HeadstoneData } from '../types/entities';
import type { ZoneData, InventoryItem } from '../types/core';
...
interface RPGHeadstoneApp {
  ...
  readonly getHeadstoneData: () => HeadstoneData;
}
...
export class RPGDeathSystem extends System {
  ...
  async init(): Promise<void> {
    ...
    this.world.on(EventType.INVENTORY_UPDATED, (data: { playerId: string; items: unknown[] }) => {
      const inventory = this.playerInventories.get(data.playerId) || { items: [], coins: 0 };
      inventory.items = data.items as InventoryItem[];
      this.playerInventories.set(data.playerId, inventory);
    });

    this.world.on(EventType.INVENTORY_COINS_UPDATED, (data: { playerId: string; newAmount: number }) => {
      const inventory = this.playerInventories.get(data.playerId) || { items: [], coins: 0 };
  ...
  start(): void {
    ...
    if (!worldGen) {
      throw new Error('[RPGDeathSystem] RPGWorldGenerationSystem is required');
    }
    this.worldGeneration = worldGen as RPGWorldGenerationSystem;
    
    const entityMgr = this.world.getSystem('rpg-entity-manager');
    if (!entityMgr) {
      throw new Error('[RPGDeathSystem] RPGEntityManager is required');
    }
    this.entityManager = entityMgr as RPGEntityManager;
  }
  ...
  private handlePlayerDeath(data: { entityId: string; killedBy: string; entityType: 'player' | 'mob' }): void {
    ...
    if (!position) {
      throw new Error(`[RPGDeathSystem] Player ${playerId} has no position`);
    }
  ...
  private createHeadstone(playerId: string, position: { x: number; y: number; z: number }, items: InventoryItem[], killedBy: string): void {
    ...
    const headstoneApp: RPGHeadstoneApp = {
      init: async () => { 
  ...
  private initiateRespawn(playerId: string): void {
    ...
    if (!deathData) {
      throw new Error(`[RPGDeathSystem] No death data found for player ${playerId}`);
    }
  ...
  private handleLootCollection(data: { playerId: string }): void {
    ...
    if (!playerPosition) {
      console.error(`[RPGDeathSystem] Could not get position for player ${data.playerId}`);
      return;
    }
