...
export class RPGItemActionTestSystem extends RPGSystemBase {
  ...
  private async createTestPlayer(playerId: string, position: { x: number; y: number; z: number }): Promise<void> {
    ...
    if (!this.world.entities.has(playerId)) {
      ...
      RPGLogger.system('RPGItemActionTestSystem', `Successfully registered fake player ${playerId} with world entities`);
    }
    ...
    RPGLogger.system('RPGItemActionTestSystem', `Created test player ${playerId} at position`, { position });
  }
  ...
  private completeActionTest(testId: string): void {
    ...
    if (results.success) {
      ...
      RPGLogger.systemError('RPGItemActionTestSystem', `❌ Test ${testId} FAILED`, undefined, { results });
      this.updateTestAreaColor(`action_test_${testId}`, 0xFF0000);
      
      // Log warning instead of throwing to prevent server crashes
      RPGLogger.systemWarn('RPGItemActionTestSystem', `Item action test ${testId} failed: ${results.errors.join(', ')}`);
    }
  }
  ...
  private failActionTest(testId: string, reason: string): void {
    ...
    RPGLogger.systemError('RPGItemActionTestSystem', `❌ Test ${testId} FAILED: ${reason}`);
    this.updateTestAreaColor(`action_test_${testId}`, 0xFF0000);
  ...
  private recordError(testId: string, error: string): void {
    ...
    RPGLogger.systemError('RPGItemActionTestSystem', `Error in test ${testId}: ${error}`);
  }
  ...
  private handleItemRightClick(data: { playerId: string; itemId: string }): void {
    ...
    // Don't simulate menu - let the actual RPGItemActionSystem handle it
  }
