...
import { Position3D, RPGMobData } from '../types/core';
import { MobType } from '../types/index';
import type { Player } from '../types/test';
import type { RPGAggroSystem } from './RPGAggroSystem';
...
export class RPGAggroTestSystem extends RPGVisualTestFramework {
  ...
  private aggroSystem!: RPGAggroSystem;
  ...
  async init(): Promise<void> {
    ...
    this.aggroSystem = getSystem(this.world, 'rpg-aggro') as RPGAggroSystem;
  ...
  private async runLowLevelAggroTest(stationId: string): Promise<void> {
    try {
      console.log(`[RPGAggroTestSystem] Starting runLowLevelAggroTest for ${stationId}`);
      const stationPosition = this.validateStationPosition(stationId);
      console.log(`[RPGAggroTestSystem] Station position received:`, stationPosition);
      ...
      if (typeof stationPosition.x !== 'number' || typeof stationPosition.y !== 'number' || typeof stationPosition.z !== 'number') {
        console.error(`[RPGAggroTestSystem] Invalid station position coordinates:`, stationPosition);
        this.failTest(stationId, `Invalid station position: ${JSON.stringify(stationPosition)}`);
        return;
      }

      console.log(`[RPGAggroTestSystem] Creating fake player at position:`, { x: stationPosition.x - 8, y: stationPosition.y, z: stationPosition.z });
      ...
      setTimeout(() => {
        console.log(`[RPGAggroTestSystem] Beginning aggro monitoring for ${stationId}`);
        this.monitorAggro(stationId);
      }, 1000);
      
    } catch (error) {
      console.error(`[RPGAggroTestSystem] Low level aggro test caught error:`, error);
      console.error(`[RPGAggroTestSystem] Error stack:`, error instanceof Error ? error.stack : 'No stack trace');
  ...
  private async runHighLevelAggroTest(stationId: string): Promise<void> {
    try {
      ...
      const stationPosition = this.validateStationPosition(stationId);
      if (!stationPosition) {
        console.error(`[RPGAggroTestSystem] Station position validation failed for ${stationId}`);
        return;
      }

      // Verify mob system is available
      if (!this.mobSystem) {
        console.error(`[RPGAggroTestSystem] MobSystem is not available`);
        this.failTest(stationId, 'MobSystem not available for spawning mobs');
        return;
      }

      // Create high-level fake player (level 25)
      console.log(`[RPGAggroTestSystem] Creating high-level fake player`);
      const player = this.createPlayer({
      ...
      const mobPosition = { x: stationPosition.x + 2, y: stationPosition.y, z: stationPosition.z };
      console.log(`[RPGAggroTestSystem] Spawning goblin at position:`, mobPosition);
      ...
      try {
        ...
        console.log(`[RPGAggroTestSystem] Mob spawned with ID: ${mobId}`);
      } catch (spawnError) {
        console.error(`[RPGAggroTestSystem] Error spawning mob:`, spawnError);
        this.failTest(stationId, `Failed to spawn mob: ${spawnError}`);
        return;
      }

      if (!mobId) {
        console.error(`[RPGAggroTestSystem] Mob spawn returned null`);
        this.failTest(stationId, 'Failed to spawn aggressive mob for high level test');
        return;
      }

      // Store test data
      console.log(`[RPGAggroTestSystem] Storing test data for ${stationId}`);
      this.testData.set(stationId, {
        player,
        mobId,
        mobType: MobType.GOBLIN,
      ...
      console.log(`[RPGAggroTestSystem] Scheduling player movement in 2 seconds`);
      setTimeout(() => {
        console.log(`[RPGAggroTestSystem] Moving player closer to mob`);
      ...
      console.log(`[RPGAggroTestSystem] Starting aggro monitoring after delay`);
      // Add delay to ensure mob is fully spawned before monitoring
      setTimeout(() => {
        console.log(`[RPGAggroTestSystem] Beginning aggro monitoring for ${stationId}`);
        this.monitorAggro(stationId);
      ...
      console.error(`[RPGAggroTestSystem] High level aggro test error:`, error);
  ...
  private monitorAggro(stationId: string): void {
    const testData = this.testData.get(stationId);
    if (!testData) {
      console.error(`[RPGAggroTestSystem] No test data found for ${stationId}`);
      return;
    }
    
    console.log(`[RPGAggroTestSystem] Starting aggro monitoring for ${stationId}`);
    let checkCount = 0;
    ...
    const checkInterval = setInterval(async () => {
      ...
      if (checkCount % 5 === 0) {
        console.log(`[RPGAggroTestSystem] Monitoring ${stationId}: elapsed=${elapsed}ms, aggro=${testData.aggroDetected}, chase=${testData.chaseStarted}`);
      }
      
      // Get current mob state
      let mob: RPGMobData | undefined = undefined;
      if (this.mobSystem) {
      ...
      if (!mob) {
        ...
        if (checkCount < 3) {
          console.warn(`[RPGAggroTestSystem] Mob ${testData.mobId} not found yet, will retry (attempt ${checkCount}/3)`);
          return; // Continue checking
        }
        
        console.error(`[RPGAggroTestSystem] Mob ${testData.mobId} not found after ${checkCount} checks - mob may have despawned`);
      ...
      const mobData = mob as RPGMobData;
      const isAggressive = mobData.aiState === 'attacking' || mobData.aiState === 'chasing';
      const hasTarget = mobData.target === testData.player.id;
      
      if (checkCount % 5 === 0) {
        console.log(`[RPGAggroTestSystem] Mob state: aiState=${mobData.aiState}, target=${mobData.target}, isAggressive=${isAggressive}`);
      }
      
      if (isAggressive && hasTarget && !testData.aggroDetected) {
        console.log(`[RPGAggroTestSystem] Aggro detected for ${stationId}!`);
      ...
      if (testData.aggroDetected && !testData.chaseStarted) {
        if (mobData.position && typeof mobData.position.x === 'number' && typeof mobData.position.y === 'number' && typeof mobData.position.z === 'number') {
          ...
          if (mobDistance > 2) { // Mob moved from spawn
            console.log(`[RPGAggroTestSystem] Chase started for ${stationId}! Distance from spawn: ${mobDistance}`);
      ...
      if (this.shouldCompleteTest(stationId, testData, elapsed)) {
        console.log(`[RPGAggroTestSystem] Test ${stationId} should complete. Elapsed: ${elapsed}ms`);
        clearInterval(checkInterval);
      ...
      if (elapsed > (station?.timeoutMs || 30000)) {
        console.error(`[RPGAggroTestSystem] Test ${stationId} timed out after ${elapsed}ms`);
        clearInterval(checkInterval);
