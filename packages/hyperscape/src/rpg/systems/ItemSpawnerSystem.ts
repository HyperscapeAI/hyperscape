...
import type { RPGEntityManager } from './RPGEntityManager';
import type { EntityManager } from '../../core/systems/EntityManager';

// Equipment loot item interface - extends RPGItem with loot-specific properties
  interface LootItem extends RPGItem {
    quantity: number;
    rarity: ItemRarity;
  }

// Item stats interface
interface ItemSpawnerStats {
  totalItems: number;
  shopItems: number;
 ...
 */
export class ItemSpawnerSystem extends RPGSystemBase {
  ...
  constructor(world: World) {
    super(world, {
      name: 'item-spawner',
      dependencies: {
        required: ['rpg-entity-manager'], // Depends on EntityManager to spawn items
        optional: ['rpg-world-generation', 'rpg-inventory', 'rpg-loot', 'rpg-store'] // Better with world generation and item systems
      },
      autoCleanup: true
    });
  }

  // Helper method to convert RPGItem to LootItem
  private toLootItem(item: RPGItem, quantity = 1, rarity?: ItemRarity): LootItem {
    return {
      ...item,
      quantity,
  ...
  private async spawnItemFromData(itemData: RPGItem, position: { x: number; y: number; z: number }, spawnType: string, location: string): Promise<string> {
