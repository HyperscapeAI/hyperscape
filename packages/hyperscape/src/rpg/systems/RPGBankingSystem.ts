...
import { BankCloseData, BankDepositAllData, BankDepositData, BankOpenData, BankWithdrawData, RPGBankData, InventoryItem } from '../types/core';
 ...
 */
export class RPGBankingSystem extends RPGSystemBase {
  private playerBanks = new Map<PlayerID, Map<BankID, RPGBankData>>(); // playerId -> bankId -> bankData
  ...
  // Logger is inherited from RPGSystemBase, no need to override
  ...
  async init(): Promise<void> {
    ...
    this.subscribe(EventType.PLAYER_REGISTERED, (event) => this.initializePlayerBanks(event.data as unknown as { id: string }));
    this.subscribe(EventType.PLAYER_UNREGISTERED, (event) => {
      const data = event.data as unknown as { id: string };
      this.cleanupPlayerBanks(data.id);
    });
    this.subscribe(EventType.BANK_OPEN, (event) => this.openBank(event.data as unknown as BankOpenData));
    this.subscribe(EventType.BANK_CLOSE, (event) => this.closeBank(event.data as unknown as BankCloseData));
    this.subscribe(EventType.BANK_DEPOSIT, (event) => this.depositItem(event.data as unknown as BankDepositData));
    this.subscribe(EventType.BANK_WITHDRAW, (event) => this.withdrawItem(event.data as unknown as BankWithdrawData));
    this.subscribe(EventType.BANK_DEPOSIT_ALL, (event) => this.depositAllItems(event.data as unknown as BankDepositAllData));

    // Listen to inventory updates for reactive pattern
    this.subscribe(EventType.INVENTORY_UPDATED, (event) => {
      const data = event.data as { playerId: string; items: InventoryItem[] };
    ...
    this.subscribe(EventType.INVENTORY_COINS_UPDATED, (event) => {
      const data = event.data as { playerId: string; newAmount: number };
      const playerId = createPlayerID(data.playerId);
      const inventory = this.playerInventories.get(playerId) || { items: [], coins: 0 };
  ...
  private initializePlayerBanks(playerData: { id: string }): void {
    ...
    const playerBanks = new Map<BankID, RPGBankData>();
  ...
  private openBank(data: BankOpenData): void {
    ...
    if (!playerBanks) {
      console.warn('[RPGBankingSystem] No banks initialized for player:', playerId);
      return;
    }
  ...
  private closeBank(data: BankCloseData): void {
    const playerId = createPlayerID(data.playerId);
  ...
  private depositItem(data: BankDepositData): void {
    ...
    if (!bankId) {
      console.warn('[RPGBankingSystem] No bank open for player:', playerId);
  ...
  private withdrawItem(data: BankWithdrawData): void {
    const playerId = createPlayerID(data.playerId);
  ...
  private depositAllItems(data: BankDepositAllData): void {
    const playerId = createPlayerID(data.playerId);
    ...
    const bank = playerBanks?.get(bankId);
    if (!bank) {
      console.error('[RPGBankingSystem] Bank not found:', bankId, 'for player:', playerId);
      return;
    }
