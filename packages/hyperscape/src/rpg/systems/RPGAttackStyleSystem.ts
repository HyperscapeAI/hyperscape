 ...
 */
export class RPGAttackStyleSystem extends RPGSystemBase {
  ...
  async init(): Promise<void> {
    ...
    this.subscribe<{ id: string }>(EventType.PLAYER_REGISTERED, (event) => this.handlePlayerRegister(event.data));
  ...
  private handleXPCalculation(data: { playerId: string; baseXP: number; skill: string; callback: (xpAmount: number) => void }): void {
    ...
    try {
      ...
      console.error(`[RPGAttackStyleSystem] Error calling XP callback for player ${playerId}:`, error);
    }
  ...
  private handleDamageCalculation(data: { playerId: string; baseDamage: number; callback: (damage: number) => void }): void {
    ...
    if (!playerState) {
      try {
        ...
        console.error(`[RPGAttackStyleSystem] Error calling damage callback for player ${playerId}:`, error);
      }
      return;
    }
  ...
  private handleAccuracyCalculation(data: { playerId: string; baseAccuracy: number; callback: (accuracy: number) => void }): void {
    ...
    if (!playerState) {
      try {
        ...
        console.error(`[RPGAttackStyleSystem] Error calling accuracy callback for player ${playerId}:`, error);
      }
      return;
    }
  ...
  private handleGetStyleInfo(data: { playerId: string; callback?: (info: Record<string, unknown> | null) => void }): void {
    ...
    if (!data.callback) {
      console.warn(`[RPGAttackStyleSystem] UI_ATTACK_STYLE_GET event received without callback for player ${playerId}`);
      this.fallbackToUpdateEvent(playerId);
      return;
    }
    
    if (typeof data.callback !== 'function') {
      console.warn(`[RPGAttackStyleSystem] UI_ATTACK_STYLE_GET event received with invalid callback for player ${playerId}. Type: ${typeof data.callback}, Value:`, data.callback);
      this.fallbackToUpdateEvent(playerId);
      return;
    }
    ...
    if (!playerState) {
      try {
        ...
        console.error(`[RPGAttackStyleSystem] Error calling callback for player ${playerId}:`, error);
      }
      return;
    }
