...
import { getWorldNetwork } from '../../../core/utils/SystemUtils';
...
export class CombatAnimationManager {
  ...
  private onAnimationComplete(entityId: string, animation: AnimationTask): void {
    ...
    if (entity && entity instanceof RPGEntity) {
   ...
   */
  private determineAnimation(entity: RPGEntity, attackType: AttackType, _style: CombatStyle): string {
    switch (attackType) {
  ...
  queueAnimation(
    ...
    const entity = this.world.entities.get(entityId);
    const animationName = entity && entity instanceof RPGEntity ? 
