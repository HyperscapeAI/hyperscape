...
import { MobEntity, NPCEntity, ResourceEntity, WorldChunk } from '../types/core';
...
export class RPGWorldContentSystem extends RPGSystemBase {
  ...
  private async generateMobSpawns(chunk: WorldChunk): Promise<void> {
    ...
    for (const spawnData of area.mobSpawns) {
      ...
      if (!mobData) {
        RPGLogger.systemWarn('RPGWorldContentSystem', `Unknown mob ID: ${spawnData.mobId}`);
        continue;
      }
  ...
  private async createMobEntity(mobData: MobData, spawnPoint: MobSpawnPoint, area: WorldArea): Promise<MobEntity | null> {
    ...
    // Convert MobData to a partial RPGMobData for compatibility
    const rpgMobData: Partial<RPGMobData> = {
      id: mobData.id,
      type: mobData.mobType as RPGMobData['type'],
      name: mobData.name,
      level: mobData.difficultyLevel,
      health: mobData.stats.health,
      maxHealth: mobData.stats.health,
      position: { x, y: spawnPoint.position.y, z },
      isAlive: true,
      isAggressive: mobData.behavior.aggressive,
      aggroRange: mobData.behavior.aggroRange,
      wanderRadius: 10,
      respawnTime: 300,
      spawnLocation: { x, y: spawnPoint.position.y, z },
      stats: mobData.stats
    };
    
    return {
      id: `mob_${mobData.id}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
      mobData: rpgMobData as RPGMobData,
      mesh: mesh as unknown as THREE.Object3D,
      area: area,
  ...
  private respawnMob(mob: MobEntity): void {
    mob.isAlive = true;
    mob.currentHealth = mob.mobData.maxHealth;
  ...
  public preparePlayerSpawnLocation(playerId: string): { x: number; y: number; z: number } {
    ...
    RPGLogger.system('RPGWorldContentSystem', `Prepared spawn location for player ${playerId} at ${area?.name || 'unknown area'} (${spawnPoint.x}, ${spawnPoint.y}, ${spawnPoint.z})`);
