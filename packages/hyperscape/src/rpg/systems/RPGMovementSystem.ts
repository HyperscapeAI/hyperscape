...
import { SystemDependencies } from '../../core/systems/System';
...
export class RPGMovementSystem extends RPGSystemBase {
  ...
  async init(): Promise<void> {
    ...
    if (!this.entityManager) {
      throw new Error('[RPGMovementSystem] EntityManager not found - required dependency');
    }
    
    // Set up event subscription for movement
    this.subscribe(EventType.MOVEMENT_CLICK_TO_MOVE, (event) => {
      this.handleClickToMove(event.data as unknown as ClickToMoveEvent);
    });
  ...
  teleportPlayer(
    ...
    if (!player.node.position) {
      RPGLogger.systemError('RPGMovementSystem', `Player ${playerId} has no node.position - this should not happen`);
      return false;
    }
  ...
  moveEntity(entityId: string, targetPosition: Position3D): void {
    ...
    if (!entity.node?.position) {
      RPGLogger.systemError('RPGMovementSystem', `Entity ${entityId} has no node.position - this should not happen`);
      return;
    }
