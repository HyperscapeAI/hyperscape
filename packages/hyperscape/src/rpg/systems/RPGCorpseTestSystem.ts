...
export class RPGCorpseTestSystem extends RPGSystemBase {
  ...
  private waitForSceneAndRunTests(): void {
    if (!this.world.stage || !this.world.stage.scene) {
      RPGLogger.system('RPGCorpseTestSystem', 'Waiting for scene to be ready...');
      setTimeout(() => this.waitForSceneAndRunTests(), 100);
      return;
    }
    
    RPGLogger.system('RPGCorpseTestSystem', 'Scene ready, starting tests');
    this.runAllTests();
  }
  ...
  private async testCorpseInteraction(): Promise<void> {
    ...
    try {
      ...
      setTimeout(() => {
        RPGLogger.system('RPGCorpseTestSystem', 'Starting corpse interaction test after delay');
  ...
  private async simulateMobDeath(testId: string, mobType: string, position: { x: number; y: number; z: number }): Promise<void> {
    RPGLogger.system('RPGCorpseTestSystem', `simulateMobDeath called for test: ${testId}`);
    ...
    const corpseId = `corpse_${testId}_${timestamp}_${random}`;
          ...
          RPGLogger.system('RPGCorpseTestSystem', `Creating corpse with ID: ${corpseId}`);
    
    const testData = this.testData.get(testId);
    if (testData) {
      testData.corpseId = corpseId;
      RPGLogger.system('RPGCorpseTestSystem', `Set corpseId in test data: ${corpseId}`);
    } else {
      RPGLogger.systemError('RPGCorpseTestSystem', `Test data not found for: ${testId}`);
    }
    ...
    RPGLogger.system('RPGCorpseTestSystem', `Created corpse visual: ${corpseVisual ? 'success' : 'failed'}`);
  ...
  private createCorpseVisual(corpseId: string, position: { x: number; y: number; z: number }, mobType: string): THREE.Mesh {
    ...
    if (this.world.stage && this.world.stage.scene) {
      ...
      RPGLogger.system('RPGCorpseTestSystem', `Corpse added to scene: ${corpseId}`);
    } else {
      RPGLogger.systemError('RPGCorpseTestSystem', `Failed to add corpse to scene - scene not available`);
    }
    
    RPGLogger.system('RPGCorpseTestSystem', `Corpse visual created at position: ${position.x}, ${position.y}, ${position.z}`);
    return corpse;
  }
  ...
  private testCorpseClick(testId: string): void {
    ...
    RPGLogger.system('RPGCorpseTestSystem', `testCorpseClick for test: ${testId}, corpseId: ${testData.corpseId}`);
    
    // First verify the corpse exists before trying to click it
    const existingCorpse = this.findCorpseById(testData.corpseId);
    if (!existingCorpse) {
      RPGLogger.systemError('RPGCorpseTestSystem', `Corpse does not exist yet! ID: ${testData.corpseId}`);
      this.recordError(testId, 'Corpse not found in scene before click attempt');
      return;
    }
    
          RPGLogger.system('RPGCorpseTestSystem', 'Corpse exists, proceeding with click test');
    ...
    // Simulate player clicking on corpse
          RPGLogger.system('RPGCorpseTestSystem', `Emitting CORPSE_CLICK event with corpseId: ${testData.corpseId}`);
    ...
    setTimeout(() => {
      RPGLogger.system('RPGCorpseTestSystem', `Checking if corpse interaction worked for: ${testData.corpseId}`);
      
      // Check if interaction worked
      const corpse = this.findCorpseById(testData.corpseId);
      if (corpse) {
        RPGLogger.system('RPGCorpseTestSystem', `Found corpse, userData.clicked: ${corpse.userData.clicked}`);
        if (corpse.userData.clicked) {
          testData.corpseInteractable = true;
          RPGLogger.system('RPGCorpseTestSystem', 'Corpse interaction successful!');
        } else {
          RPGLogger.systemError('RPGCorpseTestSystem', 'Corpse found but not marked as clicked');
          this.recordError(testId, 'Corpse click interaction did not register');
        }
      } else {
        RPGLogger.systemError('RPGCorpseTestSystem', `Could not find corpse by ID: ${testData.corpseId}`);
        this.recordError(testId, 'Corpse click interaction did not register');
      }
    }, 1000);
  }
  ...
  private verifyMultipleCorpses(testId: string): void {
    ...
    if (allTestData.length === 0) {
      RPGLogger.systemError('RPGCorpseTestSystem', `No test data found for multiple corpses test: ${testId}`);
      return;
    }
  ...
  private completeCorpseTest(testId: string): void {
    ...
    if (results.success) {
      ...
      RPGLogger.systemError('RPGCorpseTestSystem', `❌ Test ${testId} FAILED:`, undefined, { results });
      this.updateTestPlatformColor(`corpse_test_${testId}`, 0xFF0000);
      
      // Log warning instead of throwing to prevent server crashes
      RPGLogger.systemWarn('RPGCorpseTestSystem', `Corpse test ${testId} failed: ${results.errors.join(', ')}`);
    }
  }
  ...
  private failCorpseTest(testId: string, reason: string): void {
    ...
    RPGLogger.systemError('RPGCorpseTestSystem', `❌ Test ${testId} FAILED: ${reason}`);
    this.updateTestPlatformColor(`corpse_test_${testId}`, 0xFF0000);
  ...
  private recordError(testId: string, error: string): void {
    ...
    if (testData) {
      testData.errors.push(error);
          ...
          RPGLogger.systemError('RPGCorpseTestSystem', `Error in test ${testId}: ${error}`);
  }
  ...
  private handleCorpseInteraction(data: Record<string, unknown>): void {
    RPGLogger.system('RPGCorpseTestSystem', 'handleCorpseInteraction called with', { data });
    ...
    if (corpse) {
      RPGLogger.system('RPGCorpseTestSystem', `Found corpse: ${corpseId}, marking as clicked`);
      corpse.userData.clicked = true;
    } else {
      RPGLogger.systemError('RPGCorpseTestSystem', `Could not find corpse with ID: ${corpseId}`);
      ...
      if (this.world.stage.scene) {
        ...
        RPGLogger.system('RPGCorpseTestSystem', 'Available corpses in scene', { corpses });
      }
    }
  }
  ...
  private findCorpseById(corpseId: string): THREE.Object3D | null {
    if (!this.world.stage || !this.world.stage.scene) {
      RPGLogger.systemError('RPGCorpseTestSystem', `Cannot find corpse ${corpseId} - scene not available`);
      return null;
    }
    
    const obj = this.world.stage.scene.getObjectByName(corpseId);
    if (!obj) {
      RPGLogger.system('RPGCorpseTestSystem', `Corpse ${corpseId} not found in scene`);
      return null;
    }
    
    RPGLogger.system('RPGCorpseTestSystem', `Found corpse ${corpseId} in scene`);
