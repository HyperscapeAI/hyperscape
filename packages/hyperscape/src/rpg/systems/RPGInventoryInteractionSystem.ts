...
import { DragData, DropTarget, ItemType, RPGItem } from '../types/core';
...
/**
 * Create a minimal RPGItem with all required properties
 */
function createMinimalRPGItem(id: string, name: string, type: ItemType = ItemType.MISC): RPGItem {
  return {
    id,
    name,
    type,
...
export class RPGInventoryInteractionSystem extends RPGSystemBase {
  private currentDrag?: DragData;
  private dropTargets: Map<string, DropTarget> = new Map();
  private dragPreview?: HTMLElement;
  private isDragging: boolean = false;
  private playerEquipment = new Map<string, Record<string, RPGItem>>();
  ...
  async init(): Promise<void> {
    ...
    this.subscribe(EventType.PLAYER_EQUIPMENT_CHANGED, (event) => {
      const data = event.data as { playerId: string; slot: string; itemId: string | null };
      ...
      if (data.itemId) {
        ...
        equipment[data.slot] = createMinimalRPGItem(data.itemId, data.itemId, ItemType.MISC);
      } else {
  ...
  /**
   * Create a complete RPGItem from partial data
   */
  private createCompleteItem(item: Partial<RPGItem>): RPGItem {
    return {
      id: item.id || '',
   ...
   */
  private getItemData(itemId: string): RPGItem {
  ...
  private startDrag(dragData: DragData): void {
    ...
    RPGLogger.system('RPGInventoryInteractionSystem', 'Drag started', {
  ...
  private handleDropEvent(event: DragEvent, target: DropTarget): void {
    ...
    if (!this.canDropOnTarget(this.currentDrag.itemData, target)) {
      RPGLogger.systemWarn('RPGInventoryInteractionSystem', `Invalid drop: ${this.currentDrag.itemData.name} cannot be dropped on ${target.type} slot ${target.slot}`);
      this.cancelDrag();
      return;
    }
  ...
  private performDrop(dragData: DragData, target: DropTarget): void {
    ...
    if (dragData.sourceType === 'inventory' && target.type === 'equipment') {
      ...
      RPGLogger.system('RPGInventoryInteractionSystem', 'Equipment swap', {
        playerId: this.getCurrentPlayerId(),
        fromSlot: dragData.sourceSlot,
        toSlot: target.slot
      });
  ...
  private endDrag(): void {
    if (this.currentDrag) {
      ...
      RPGLogger.system('RPGInventoryInteractionSystem', 'Drag ended', {
  ...
  private canDropOnTarget(
    ...
    if (target.type === 'equipment') {
      ...
      const item = itemData as RPGItem;
  ...
  private getInventoryItem(playerId: string, slot: number): RPGItem | null {
    // Use event-based approach to get inventory item
     
    let inventoryItem: RPGItem | null = null;
  ...
  private getEquipmentItem(playerId: string, slot: string): RPGItem | null {
