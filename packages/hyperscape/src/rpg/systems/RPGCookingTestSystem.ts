/**
 * RPG Cooking Test System - REAL, RUNTIME, NO-MOCK, GDD-COMPLIANT
 * Tests the complete, real cooking loop by listening to events from RPGProcessingSystem.
...
import type { CookingTestData, Player } from '../types/test'
...
export class RPGCookingTestSystem extends RPGVisualTestFramework {
  ...
  private async runCookingTest(stationId: string, cookingLevel: number, attempts: number, expectedSuccessRate: number): Promise<void> {
    try {
      ...
      RPGLogger.system('RPGCookingTestSystem', `Created fake player: ${player.id}, fakePlayers size after create: ${this.fakePlayers.size}`);
  ...
  private async createFire(playerId: string, _position: {x:number, y:number, z:number}): Promise<{fireId: string}> {
      return new Promise((resolve, reject) => {
          ...
          RPGLogger.system('RPGCookingTestSystem', `Looking for tinderbox with key: ${tinderboxKey}`);
          RPGLogger.system('RPGCookingTestSystem', `Looking for logs with key: ${logsKey}`);
          
          const tinderbox = getItem(tinderboxKey);
          const logs = getItem(logsKey);
          
          RPGLogger.system('RPGCookingTestSystem', `Tinderbox found: ${tinderbox ? 'YES' : 'NO'}`);
          RPGLogger.system('RPGCookingTestSystem', `Logs found: ${logs ? 'YES' : 'NO'}`);
          
          let player = this.getPlayer(playerId);
          RPGLogger.system('RPGCookingTestSystem', `Fake player found: ${player ? 'YES' : 'NO'}, playerId: ${playerId}`);
          RPGLogger.system('RPGCookingTestSystem', `Fake players map size: ${this.fakePlayers.size}`);
          RPGLogger.system('RPGCookingTestSystem', 'Fake players IDs', { ids: Array.from(this.fakePlayers.keys()) });
          
          // If fake player not found, wait and retry once
          if (!player) {
              RPGLogger.system('RPGCookingTestSystem', 'Fake player not found, waiting 500ms and retrying...');
              setTimeout(() => {
                  player = this.getPlayer(playerId);
                  RPGLogger.system('RPGCookingTestSystem', `Retry - Fake player found: ${player ? 'YES' : 'NO'}`);
  ...
  private updateInventory(player: Player, item: { id: string }, quantity: number, action: 'add' | 'remove'): void {
    if (action === 'add') {
