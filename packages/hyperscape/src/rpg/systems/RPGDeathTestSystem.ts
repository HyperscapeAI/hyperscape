...
export class RPGDeathTestSystem extends RPGVisualTestFramework {
  ...
  async init(): Promise<void> {
    ...
    this.combatSystem = this.world.systems.find((s: System) => s.constructor.name === 'RPGCombatSystem') || null;
    this.inventorySystem = this.world.systems.find((s: System) => s.constructor.name === 'RPGInventorySystem') || null;
    this.playerSystem = this.world.systems.find((s: System) => s.constructor.name === 'RPGPlayerSystem') || null;
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (currentTestData && currentTestData.headstoneLocation) {
          RPGLogger.system('RPGDeathTestSystem', 'Moving player back to headstone for retrieval');
  ...
  private startCombatWithGoblin(stationId: string): void {
    const testData = this.testData.get(stationId);
    if (!testData) {
      RPGLogger.systemWarn('RPGDeathTestSystem', `No test data found for station ${stationId}`);
      return;
    }
  ...
  private handlePlayerDeath(data: { playerId: string; cause: string; position: { x: number; y: number; z: number } }): void {
    ...
    RPGLogger.system('RPGDeathTestSystem', `Player death detected: ${data.playerId} at position ${JSON.stringify(data.position)}`);
    ...
    for (const [stationId, testData] of Array.from(this.testData.entries())) {
      if (testData.player.id === data.playerId) {
        ...
        RPGLogger.system('RPGDeathTestSystem', `Death processed for station ${stationId}`);
        break;
      }
    }
  }
  ...
  private handlePlayerRespawn(data: { playerId: string; newPosition: { x: number; y: number; z: number }; respawnTime: number }): void {
    ...
    RPGLogger.system('RPGDeathTestSystem', `Player respawn detected: ${data.playerId} at position ${JSON.stringify(data.newPosition)}`);
    ...
    for (const [stationId, testData] of Array.from(this.testData.entries())) {
      if (testData.player.id === data.playerId) {
        ...
        RPGLogger.system('RPGDeathTestSystem', `Respawn processed for station ${stationId}`);
        break;
      }
    }
  }
  ...
  private handleItemsDropped(data: { itemId: string; playerId: string; position: { x: number; y: number; z: number } }): void {
    ...
    RPGLogger.system('RPGDeathTestSystem', `Item dropped detected: ${data.itemId} for player ${data.playerId}`);
    ...
    for (const [stationId, testData] of Array.from(this.testData.entries())) {
      if (testData.player.id === data.playerId) {
        ...
        RPGLogger.system('RPGDeathTestSystem', `Item drop processed for station ${stationId}: total ${testData.itemsDropped.length} items`);
        break;
      }
    }
  }
  ...
  private handleItemsRetrieved(data: { entityId: string; playerId: string; items: Array<{ id: string; itemId: string; quantity: number; slot: number }>; totalItems: number }): void {
    ...
    RPGLogger.system('RPGDeathTestSystem', `Items retrieved detected: ${data.items.length} items for player ${data.playerId}`);
    ...
    for (const [stationId, testData] of Array.from(this.testData.entries())) {
      if (testData.player.id === data.playerId) {
        ...
        if (allItemsRetrieved) {
          RPGLogger.system('RPGDeathTestSystem', `All items successfully retrieved for station ${stationId}`);
        }
  ...
  private attemptItemRetrieval(stationId: string, _playerId: string): boolean {
    ...
    RPGLogger.system('RPGDeathTestSystem', `Attempting item retrieval for station ${stationId}`);
    ...
    if (testData.headstoneCreated && testData.headstoneLocation) {
      ...
      if (distance <= 2) { // Within interaction range
        ...
        RPGLogger.system('RPGDeathTestSystem', `Items retrieved successfully for station ${stationId}`);
        return true;
      }
    }
  ...
  private scheduleHeadstonePersistenceChecks(stationId: string): void {
    RPGLogger.system('RPGDeathTestSystem', `Scheduling headstone persistence checks for station ${stationId}`);
    
    // Schedule checks for headstone persistence
    setTimeout(() => {
      const testData = this.testData.get(stationId);
      if (testData && testData.headstoneCreated) {
        RPGLogger.system('RPGDeathTestSystem', `Headstone persisted after 30 seconds for station ${stationId}`);
      }
    }, 30000);
  }

  private completeMultipleDeathsTest(stationId: string, success: boolean, error?: string): void {
    RPGLogger.system('RPGDeathTestSystem', `Multiple deaths test ${success ? 'PASSED' : 'FAILED'} for station ${stationId}${error ? ': ' + error : ''}`);
  ...
  protected updateStationStatus(stationId: string, status: 'idle' | 'running' | 'passed' | 'failed', error?: string): void {
    ...
    if (station) {
      ...
      RPGLogger.system('RPGDeathTestSystem', `Updated station ${stationId} status to ${status}${error ? ': ' + error : ''}`);
    }
  }
