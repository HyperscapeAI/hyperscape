...
import { ItemType, RPGItem } from '../data/items';
import { GroundItem, World } from '../types/core';
import { ItemPickupSystemInfo as SystemInfo } from '../types/rpg-system-types';
import { safeSceneAdd } from '../utils/EntityUtils';
import { RPGSystemBase } from './RPGSystemBase';

export class RPGItemPickupSystem extends RPGSystemBase {
   ...
   */
  public dropItem(item: RPGItem, position: { x: number; y: number; z: number }, droppedBy: string): string {
   ...
   */
  private createItemMesh(item: RPGItem, itemId: string): THREE.Mesh {
