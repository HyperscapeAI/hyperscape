...
import type { RPGLootSystem } from './RPGLootSystem';
...
export class RPGLootDropTestSystem extends RPGVisualTestFramework {
  ...
  private verifyLootDespawned(testId: string): void {
    ...
    const lootSystem = this.world.getSystem<RPGLootSystem>('rpg-loot');
    if (lootSystem) {
  ...
  private findGroundItemsNear(position: { x: number; y: number; z: number }, radius: number = 5.0): THREE.Object3D[] {
    ...
    this.world.stage.scene.traverse((obj) => {
      ...
      const isItem = obj.userData.type === 'item' || 
                     ...
                     obj.name.includes('drop_') || // RPGLootSystem uses drop_ prefix
  ...
  // Required abstract methods from RPGVisualTestFramework
