...
import type { Inventory, InventoryItem, PlayerEquipment } from '../types/core';
...
export class RPGStoreTestSystem extends RPGVisualTestFramework {
  ...
  private storeSystem!: RPGStoreSystem;
  ...
  async init(): Promise<void> {
    ...
    this.storeSystem = getSystem<RPGStoreSystem>(this.world, 'rpg-store')!;
    this.inventorySystem = getSystem<RPGInventorySystem>(this.world, 'rpg-inventory')!;
  ...
  private async runBasicPurchaseTest(stationId: string): Promise<void> {
    ...
    const player = basePlayer as unknown as Player;
  ...
  private startPurchaseSequence(stationId: string): void {
    ...
    const purchaseNextItem = async () => {
      ...
      // Removed console.log(`[RPGStoreTestSystem] Attempting to purchase: ${testItem.itemId} x${testItem.quantity} for ${totalCost} coins`);
        ...
        if (this.storeSystem) {
          ...
          try {
            ...
            if (success) {
              ...
              // Removed console.log(`[RPGStoreTestSystem] Purchase successful for ${testItem.itemId}`);
              ...
              if (item) {
                ...
                // Removed console.error(`[RPGStoreTestSystem] Failed to get item: ${testItem.itemId}`);
                // Don't count this as a successful purchase if we can't get the item
                testData.itemsPurchased--;
              }
              
            } else {
              // Removed console.warn('[RPGStoreTestSystem] Purchase failed');
            }
          } catch (_error) {
            // Removed console.warn('[RPGStoreTestSystem] Purchase item warning:', _error);
          }
        } else {
          // Removed console.warn('[RPGStoreTestSystem] Store system or purchaseItem method not available');
        }
      } else {
        // Removed console.warn(`[RPGStoreTestSystem] Insufficient funds for purchase: need ${totalCost}, have ${availableCoins}`);
      }
        ...
        if (this.storeSystem) {
          ...
          try {
            ...
            if (success) {
              ...
              // Removed console.warn('[RPGStoreTestSystem] Sell failed');
            }
          } catch (_error) {
            // Removed console.warn('[RPGStoreTestSystem] Sell item warning:', _error);
          }
        } else {
          // Removed console.warn('[RPGStoreTestSystem] Store system or sellItem method not available');
        }
      } else {
        // Removed console.warn('[RPGStoreTestSystem] Insufficient item quantity for sale');
      }
  ...
  private startInsufficientFundsSequence(stationId: string): void {
    ...
    // Removed console.log(`[RPGStoreTestSystem] Starting insufficient funds sequence`);
        ...
        if (this.storeSystem) {
          try {
            ...
            if (coinsDeducted > 0) {
              ...
              // Removed console.log(`[RPGStoreTestSystem] Purchase successful for ${testItem.itemId}, spent ${coinsDeducted} coins`);
            } else {
              // Removed console.log(`[RPGStoreTestSystem] Purchase failed for ${testItem.itemId}, no coins deducted`);
            }
          } catch (_error) {
            // Removed console.warn(`[RPGStoreTestSystem] Purchase error:`, _error);
          }
        }
      } else {
        // Removed console.log(`[RPGStoreTestSystem] Insufficient funds: ${availableCoins} < ${totalCost}`);
      }
        ...
        if (this.storeSystem) {
          ...
          try {
            ...
            if (actuallyPurchased > 0) {
              ...
              // Removed console.log(`[RPGStoreTestSystem] Stock limit test: Requested ${requestedQuantity}, got ${actuallyPurchased} (stock was ${availableStock})`);
            }
          } catch (_error) {
            // Removed console.warn(`[RPGStoreTestSystem] Purchase error:`, _error);
          }
        } else {
          // Removed console.error('[RPGStoreTestSystem] Store system not available for stock limit test!');
        }
      }
      ...
      setTimeout(async () => {
        ...
        if (this.storeSystem) {
          ...
          try {
            ...
            // Removed console.warn('[RPGStoreTestSystem] Bulk sell error:', _error);
          }
        }
