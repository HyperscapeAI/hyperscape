...
import type {
  RPGBankDepositSuccessEvent,
  RPGInventoryCheckEvent,
  RPGInventoryItemInfo,
  RPGInventoryRemoveEvent
} from '../types/events';
import type { Player } from '../types/test';
import type { RPGBankingSystem } from './RPGBankingSystem';
import type { RPGInventorySystem } from './RPGInventorySystem';
...
export class RPGBankingTestSystem extends RPGVisualTestFramework {
  ...
  private bankingSystem: RPGBankingSystem | null = null;
  private inventorySystem: RPGInventorySystem | null = null;
  ...
  async init(): Promise<void> {
    ...
    RPGLogger.system('RPGBankingTestSystem', 'Initializing banking test system...');
    
    this.bankingSystem = this.world.getSystem('rpg-banking') as RPGBankingSystem;
    this.inventorySystem = this.world.getSystem('rpg-inventory') as RPGInventorySystem;

    if (!this.bankingSystem) {
      RPGLogger.systemError('RPGBankingTestSystem', 'Banking system not found');
      return;
    } else {
      RPGLogger.system('RPGBankingTestSystem', 'Banking system found successfully');
    }

    if (!this.inventorySystem) {
      RPGLogger.systemError('RPGBankingTestSystem', 'Inventory system not found');
      return;
    } else {
      RPGLogger.system('RPGBankingTestSystem', 'Inventory system found successfully');
    }

    // Create test stations
    this.createTestStations();
          RPGLogger.system('RPGBankingTestSystem', 'Test stations created');
    
    // Set up event listeners for banking tests
    this.setupEventListeners();
          RPGLogger.system('RPGBankingTestSystem', 'Event listeners set up');
  }

  private setupEventListeners(): void {
    // Listen for inventory updates
    this.world.on(EventType.INVENTORY_UPDATED, (data: { playerId: string; items: InventoryItem[] }) => {
    ...
    this.world.on(EventType.INVENTORY_CHECK, (event: RPGInventoryCheckEvent) => {
      for (const testData of this.testData.values()) {
        if (testData.player.id === event.playerId) {
          ...
          if (inventorySlot && inventorySlot.quantity >= event.quantity) {
            ...
            const itemInfo: RPGInventoryItemInfo = {
    ...
    this.world.on(EventType.INVENTORY_ITEM_REMOVED, (data: RPGInventoryRemoveEvent) => {
      for (const testData of this.testData.values()) {
        if (testData.player.id === data.playerId) {
  ...
  private async runBasicDepositTest(stationId: string): Promise<void> {
    try {
      RPGLogger.system('RPGBankingTestSystem', 'Starting basic deposit test...');
      const station = this.testStations.get(stationId);
      if (!station) {
        RPGLogger.systemError('RPGBankingTestSystem', `Test station not found: ${stationId}`);
        return;
      }
  ...
  private async runBasicWithdrawTest(stationId: string): Promise<void> {
    try {
      ...
      for (const [itemId, quantity] of Object.entries(bankBalance)) {
        const event: RPGBankDepositSuccessEvent = {
          playerId: player.id,
          bankId: 'bank_town_0',
          itemId,
          quantity
        };
        this.world.emit(EventType.BANK_DEPOSIT_SUCCESS, event);
      }
  ...
  private async runBankIndependenceTest(stationId: string): Promise<void> {
    try {
      RPGLogger.system('RPGBankingTestSystem', 'Starting bank independence test setup...');
      const station = this.testStations.get(stationId);
      if (!station) return;
      ...
      RPGLogger.system('RPGBankingTestSystem', 'Starting bank independence test...');
  ...
  private startDepositSequence(stationId: string): void {
    ...
    if (!testData) {
      RPGLogger.systemError('RPGBankingTestSystem', `No test data found for: ${stationId}`);
      return;
    }

    RPGLogger.system('RPGBankingTestSystem', `Starting deposit sequence for player: ${testData.player.id}`);
    ...
    setTimeout(() => {
      RPGLogger.system('RPGBankingTestSystem', `Opening bank for player: ${testData.player.id}`);
      // First open the bank
      ...
      setTimeout(() => {
        RPGLogger.system('RPGBankingTestSystem', `Attempting to deposit ${testData.testItems.length} items`);
        for (const testItem of testData.testItems) {
          RPGLogger.system('RPGBankingTestSystem', `Emitting deposit event for: ${testItem.itemId}, quantity: ${testItem.quantity}`);
        ...
        const successHandler = (data: { playerId: string; itemId: string; quantity: number }) => {
          RPGLogger.system('RPGBankingTestSystem', 'Deposit success event received', { data });
          if (data.playerId === testData.player.id) {
            ...
            RPGLogger.system('RPGBankingTestSystem', `Deposit successful! Total deposits: ${testData.depositedItems}/${testData.testItems.length}`);
            
            if (testData.depositedItems >= testData.testItems.length) {
              this.world.off(EventType.BANK_DEPOSIT_SUCCESS, successHandler);
              this.completeDepositTest(stationId);
            }
          }
        };

        this.world.on(EventType.BANK_DEPOSIT_SUCCESS, successHandler);
  ...
  private startWithdrawSequence(stationId: string): void {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        const withdrawHandler = (data: { playerId: string; itemId: string; quantity: number }) => {
          if (data.playerId === testData.player.id) {
            ...
            if (testData.withdrawnItems >= testData.testItems.length) {
              this.world.off(EventType.BANK_WITHDRAW_SUCCESS, withdrawHandler);
              this.completeWithdrawTest(stationId);
            }
          }
        };

        this.world.on(EventType.BANK_WITHDRAW_SUCCESS, withdrawHandler);
  ...
  private startBulkSequence(stationId: string): void {
    ...
    if (!testData) return;
          ...
          RPGLogger.system('RPGBankingTestSystem', 'Starting bulk banking test...');
    ...
    const bankOpenHandler = (data: { playerId: string; bankId: string }) => {
      if (data.playerId === testData.player.id) {
        RPGLogger.system('RPGBankingTestSystem', 'Bank opened successfully');
        bankOpened = true;
    ...
    // Open the bank first (required by RPGBankingSystem)
    setTimeout(() => {
      RPGLogger.system('RPGBankingTestSystem', 'Opening bank for bulk test...');
      this.emitTypedEvent(EventType.BANK_OPEN, {
        playerId: testData.player.id,
                   bankId: 'bank_town_0',
        playerPosition: { 
      ...
      setTimeout(() => {
        if (!bankOpened) {
          RPGLogger.system('RPGBankingTestSystem', 'Bank open timeout, proceeding anyway...');
          this.world.off(EventType.BANK_OPEN, bankOpenHandler);
          performDeposits();
        }
      }, 3000);
    ...
    const performDeposits = () => {
      RPGLogger.system('RPGBankingTestSystem', 'Starting bulk deposits...');
      let depositsCompleted = 0;
      const totalItems = testData.testItems.length;
      
      // Listen for deposit success events
      const depositHandler = (data: { playerId: string; itemId: string; quantity: number; bankId?: string }) => {
        if (data.playerId === testData.player.id) {
          depositsCompleted++;
          testData.depositedItems++;
          RPGLogger.system('RPGBankingTestSystem', `Deposit ${depositsCompleted}/${totalItems} completed: ${data.itemId}`);
          
          if (depositsCompleted >= totalItems) {
            this.world.off(EventType.BANK_DEPOSIT_SUCCESS, depositHandler);
            performWithdrawals();
          }
        }
      };
      ...
      const depositFailHandler = (data: { playerId: string; reason: string }) => {
        if (data.playerId === testData.player.id) {
          RPGLogger.system('RPGBankingTestSystem', `Deposit failed: ${data.reason}`);
        }
      };
      
      this.world.on(EventType.BANK_DEPOSIT_SUCCESS, depositHandler);
      this.world.on(EventType.BANK_DEPOSIT_FAIL, depositFailHandler);
      
      // Deposit all items with a small delay between each
      testData.testItems.forEach((testItem, index) => {
        setTimeout(() => {
          depositsAttempted++;
          RPGLogger.system('RPGBankingTestSystem', `Attempting deposit ${depositsAttempted}/${totalItems}: ${testItem.itemId}`);
          this.emitTypedEvent(EventType.BANK_DEPOSIT, {
            playerId: testData.player.id,
            bankId: 'bank_town_0', 
            itemId: testItem.itemId,
      ...
      setTimeout(() => {
        this.world.off(EventType.BANK_DEPOSIT_SUCCESS, depositHandler);
        this.world.off(EventType.BANK_DEPOSIT_FAIL, depositFailHandler);
        if (depositsCompleted < totalItems) {
          RPGLogger.system('RPGBankingTestSystem', `Deposit phase incomplete: ${depositsCompleted}/${totalItems}`);
        }
        performWithdrawals();
      }, 8000);
    };
    ...
    const performWithdrawals = () => {
      RPGLogger.system('RPGBankingTestSystem', 'Starting bulk withdrawals...');
      ...
      const withdrawHandler = (data: { playerId: string; itemId: string; quantity: number; bankId?: string }) => {
        if (data.playerId === testData.player.id) {
          withdrawsCompleted++;
          testData.withdrawnItems++;
          RPGLogger.system('RPGBankingTestSystem', `Withdraw ${withdrawsCompleted}/${totalItems} completed: ${data.itemId}`);
      ...
      const withdrawFailHandler = (data: { playerId: string; reason: string }) => {
        if (data.playerId === testData.player.id) {
          RPGLogger.system('RPGBankingTestSystem', `Withdraw failed: ${data.reason}`);
        }
      };
      
      this.world.on(EventType.BANK_WITHDRAW_SUCCESS, withdrawHandler);
      this.world.on(EventType.BANK_WITHDRAW_FAIL, withdrawFailHandler);
      
      // Withdraw all items with a small delay between each
      testData.testItems.forEach((testItem, index) => {
        setTimeout(() => {
          withdrawsAttempted++;
          RPGLogger.system('RPGBankingTestSystem', `Attempting withdraw ${withdrawsAttempted}/${totalItems}: ${testItem.itemId}`);
          this.emitTypedEvent(EventType.BANK_WITHDRAW, {
            playerId: testData.player.id,
            bankId: 'bank_town_0',
            itemId: testItem.itemId,
      ...
      setTimeout(() => {
        this.world.off(EventType.BANK_WITHDRAW_SUCCESS, withdrawHandler);
        this.world.off(EventType.BANK_WITHDRAW_FAIL, withdrawFailHandler);
        RPGLogger.system('RPGBankingTestSystem', `Bulk test complete - deposits: ${testData.depositedItems}, withdrawals: ${testData.withdrawnItems}`);
        this.completeBulkTest(stationId);
      }, 8000);
    };
  }
  ...
  private startIndependenceSequence(
    ...
    const depositPhase = () => {
      RPGLogger.system('RPGBankingTestSystem', 'Independence test phase 1: Deposits');
      ...
      const depositHandler = (data: { playerId: string; bankId: string }) => {
        if (data.playerId === player1.id || data.playerId === player2.id) {
          depositsCompleted++;
          RPGLogger.system('RPGBankingTestSystem', `Player deposit completed: ${data.playerId} at ${data.bankId}`);
    ...
    const crossWithdrawPhase = () => {
      RPGLogger.system('RPGBankingTestSystem', 'Independence test phase 2: Cross-withdrawals');
      ...
      const withdrawFailHandler = (data: { playerId: string; reason: string }) => {
        withdrawalAttempts++;
                  RPGLogger.system('RPGBankingTestSystem', `Expected withdrawal failure: ${data.playerId} - ${data.reason}`);
    ...
    const ownWithdrawPhase = () => {
      RPGLogger.system('RPGBankingTestSystem', 'Independence test phase 3: Own withdrawals');
      ...
      const withdrawSuccessHandler = (data: { playerId: string; bankId: string }) => {
        if (data.playerId === player1.id || data.playerId === player2.id) {
          successfulWithdrawals++;
          RPGLogger.system('RPGBankingTestSystem', `Successful withdrawal: ${data.playerId} from ${data.bankId}`);
          
          if (successfulWithdrawals >= 2) {
            this.world.off(EventType.BANK_WITHDRAW_SUCCESS, withdrawSuccessHandler);
            this.completeIndependenceTest(stationId, true);
          }
        }
      };

      this.world.on(EventType.BANK_WITHDRAW_SUCCESS, withdrawSuccessHandler);
  ...
  private startStorageLimitSequence(stationId: string): void {
    ...
    setTimeout(() => {
      ...
      let successCount = 0;
      const depositHandler = (data: { playerId: string }) => {
        if (data.playerId === testData.player.id) {
          successCount++;
