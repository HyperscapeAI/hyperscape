...
import { RPGEntity, RPGEntityConfig } from '../entities/RPGEntity';
...
import type { EntitySpawnedEvent } from '../types/rpg-systems';
import { RPGSystemBase } from './RPGSystemBase';
import { RPGLogger } from '../utils/RPGLogger';

export class RPGEntityManager extends RPGSystemBase {
  ...
  async init(): Promise<void> {
    ...
    RPGLogger.system('RPGEntityManager', '✅ Event listeners registered, ready to handle mob spawns');
  ...
  async spawnEntity(config: RPGEntityConfig): Promise<RPGEntity | null> {
    ...
    switch (config.type) {
      ...
      case 'npc':
        ...
        break;
      default:
        throw new Error(`[RPGEntityManager] Unknown entity type: ${config.type}`);
    }
    ...
    RPGLogger.system('RPGEntityManager', `Adding entity ${config.id} to world.entities`);
    this.world.entities.set(config.id, entity);
    RPGLogger.system('RPGEntityManager', `Entity ${config.id} added to world.entities`);
  ...
  private async handleInteractionRequest(data: { entityId: string; playerId: string; interactionType: string }): Promise<void> {
    ...
    if (!entity) {
      RPGLogger.system('RPGEntityManager', ` Entity not found: ${data.entityId}`);
      return;
    }
  ...
  private async handleItemSpawn(data: ItemSpawnData): Promise<void> {
    const config: RPGEntityConfig<ItemEntityProperties> = {
  ...
  private handleItemPickup(data: { entityId: string; playerId: string }): void {
    const entity = this.entities.get(data.entityId);
    if (!entity) {
      RPGLogger.system('RPGEntityManager', ` Cannot pickup entity ${data.entityId} - not found`);
      return;
    }
  ...
  private async handleMobSpawn(data: MobSpawnData): Promise<void> {
    RPGLogger.system('RPGEntityManager', 'Handling mob spawn request', { data });
    
    // Validate required data
    const position = data.position;
    if (!position) {
      throw new Error('[RPGEntityManager] Mob spawn position is required');
    }
    
    const mobType = data.mobType;
    if (!mobType) {
      throw new Error('[RPGEntityManager] Mob type is required');
    }
    ...
    RPGLogger.system('RPGEntityManager', `✅ Mob entity spawned: ${entity?.id} (${mobType})`);
    RPGLogger.system('RPGEntityManager', `Entity exists in world.entities: ${this.world.entities.has(config.id)}`);
    RPGLogger.system('RPGEntityManager', 'Entity spawned successfully');
  }
  ...
  private handleMobAttacked(data: { entityId: string; damage: number; attackerId: string }): void {
    ...
    if (!mob) {
      RPGLogger.system('RPGEntityManager', ` Cannot handle mob attacked - entity ${data.entityId} not found`);
      return;
    }
    
    const currentHealth = mob.getProperty('health');
    if (typeof currentHealth !== 'number') {
      RPGLogger.systemError('RPGEntityManager', `Invalid health value for mob ${data.entityId}`);
      return;
    }
  ...
  private handleMobAttack(data: { mobId: string; targetId: string; damage: number }): void {
    ...
    if (!mob) {
      RPGLogger.system('RPGEntityManager', ` Cannot handle mob attack - entity ${data.mobId} not found`);
      return;
    }
  ...
  private getMobMaxHealth(mobType: string, level: number): number {
    ...
    RPGLogger.system('RPGEntityManager', ` Unknown mob type: ${mobType}`);
    return 50 + (level * 10);
  }
  ...
  private async handleResourceSpawn(data: ResourceSpawnData): Promise<void> {
    const config: RPGEntityConfig<ResourceEntityProperties> = {
  ...
  private handleResourceHarvest(data: { entityId: string; playerId: string; amount: number }): void {
    ...
    if (!resource) {
      RPGLogger.system('RPGEntityManager', ` Cannot harvest resource - entity ${data.entityId} not found`);
      return;
    }
  ...
  private async handleNPCSpawn(data: NPCSpawnData): Promise<void> {
    const config: RPGEntityConfig<NPCEntityProperties> = {
  ...
  private handleNPCDialogue(data: { entityId: string; playerId: string; dialogueId: string }): void {
    ...
    if (!npc) {
      RPGLogger.system('RPGEntityManager', ` Cannot start NPC dialogue - entity ${data.entityId} not found`);
      return;
    }
  ...
  destroy(): void {
    ...
    for (const entity of this.entities.values()) {
      if (entity) {
        // Assume destroy method exists on RPG entities
        entity.destroy();
      }
    }
    this.entities.clear();
    ...
    RPGLogger.system('RPGEntityManager', 'Entity manager destroyed and cleaned up');
