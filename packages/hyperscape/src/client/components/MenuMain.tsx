...
function MenuMainIndex({ world, _pop, push }) {
  const { isAdmin: _isAdmin, isBuilder } = usePermissions(world)
...
function MenuMainUI({ world, _pop, _push }) {
  ...
  return (
    <Menu title='Menu' blur={false}>
      <MenuItemBack hint='Go back to the main menu' onClick={_pop} />
      <MenuItemToggle
        label='Fullscreen'
...
function MenuMainGraphics({ world, _pop, _push }) {
...
function MenuMainAudio({ world, _pop, _push }) {
