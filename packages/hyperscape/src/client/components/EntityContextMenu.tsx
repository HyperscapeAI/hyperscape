...
export function EntityContextMenu({ world }: EntityContextMenuProps) {
  ...
  useEffect(() => {
    ...
    const handleContextMenu = (event: Event) => {
      ...
      const actions: ContextMenuAction[] = items.map(item => ({
        ...
        onClick: () => {
          ...
          const selectEvent = new CustomEvent('rpg:contextmenu:select', {
            detail: {
              actionId: item.id,
              targetId: target.id
            }
          ...
          console.log('[EntityContextMenu] Dispatched rpg:contextmenu:select event');
    ...
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (target.closest('.rpg-context-menu')) {
    ...
    window.addEventListener('rpg:contextmenu', handleContextMenu as EventListener)
    window.addEventListener('rpg:contextmenu:close', handleClose as EventListener)
    // Use click (not mousedown) to let onClick handlers fire first
    document.addEventListener('click', handleClickOutside, false)

    return () => {
      window.removeEventListener('rpg:contextmenu', handleContextMenu as EventListener)
      window.removeEventListener('rpg:contextmenu:close', handleClose as EventListener)
  ...
  return (
    <div
      className="rpg-context-menu fixed bg-[rgba(20,20,20,0.95)] border border-[#555] rounded pointer-events-auto z-[99999]"
      style={{
