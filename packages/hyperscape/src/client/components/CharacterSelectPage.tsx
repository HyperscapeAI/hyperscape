...
export function CharacterSelectPage({
  ...
  React.useEffect(() => {
    const onStorage = (e: StorageEvent) => {
      if (!e.key) return
      if (e.key === 'privy_auth_token' || e.key === 'privy_user_id') {
  ...
  React.useEffect(() => {
    ...
    ws.addEventListener('message', (e) => {
      try {
        ...
        if (!result) return
        const [method, data] = result as [string, any]
        ...
        if (method === 'onSnapshot') {
          ...
          if (evt?.name === 'rpg:character:list') {
            const list = (evt.data as { characters?: Character[] })?.characters || []
            try { console.log('[CharacterSelect] onEntityEvent rpg:character:list:', list.length) } catch {}
            setCharacters(list)
          }
        }
      } catch {}
