 ...
 * 
 * Extends Entity to provide storage-specific functionality including:
...
import type { EntityConfig, EntityInteractionData, BankEntityProperties } from '../types/entities';
import type { Item, InventoryItem } from '../types/core';
import { ItemType } from '../types/core';
import { ItemRarity } from '../types/entities';

export interface StorageConfig extends EntityConfig<BankEntityProperties> {
  storage?: {
...
export abstract class StorageEntity extends Entity {
  ...
  public storeItems(items: InventoryItem[], playerId?: string): {
    ...
    for (const item of items) {
      // Check if item is allowed (convert to Item for checking)
      const baseItem: Item = { 
        id: item.itemId, 
        name: '', 
        type: ItemType.MISC, 
   ...
   */
  protected canStoreItem(item: Item): boolean {
    // Check blocked items
    ...
    // This would require expanding the Item interface to include category
