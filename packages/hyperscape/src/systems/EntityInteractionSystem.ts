...
export class EntityInteractionSystem extends SystemBase {
  ...
  private onMouseDown(event: MouseEvent): void {
    if (event.button === 2) { // Right-click
      ...
      window.dispatchEvent(new CustomEvent('rpg:contextmenu:close'));
    }
  }
  ...
  private showContextMenu(target: { id: string; type: string; name: string; entity: any; position: Position3D }, screenX: number, screenY: number): void {
    ...
    const onSelect = (e: Event) => {
      ...
      console.log('[EntityInteractionSystem] Received rpg:contextmenu:select:', ce.detail);
  ...
  private getActionsForEntityType(target: { id: string; type: string; name: string; entity: any; position: Position3D }, playerId: string): InteractionAction[] {
  ...
  private examineEntity(target: { type: string; name: string; entity: any }, playerId: string): void {
