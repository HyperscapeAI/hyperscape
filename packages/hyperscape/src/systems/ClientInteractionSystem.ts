...
import type { World, Player } from '../types/index';
import type { ClientControls } from './ClientControls';
import type {
  AvatarReadyEvent,
...
export class ClientInteractionSystem extends SystemBase {
  ...
  private tryInitialize(): void {
    // Get local player - try multiple methods
      ...
      if (player) {
        this.localPlayer = player as Player;
      }
  ...
  private onMouseClick(event: MouseEvent): void {
    ...
    const userData = clickedObject.object.userData as UserData;
  ...
  private handlePlayerClick(_clickedObject: THREE.Intersection, _userData: UserData): void {
    // For now, clicking on players does nothing (no PvP in MVP)
    // Could show player info or trade window in future
    
  }

  private handleCorpseClick(clickedObject: THREE.Intersection, userData: UserData): void {
    const playerId = this.localPlayer.id;
  ...
  private updateCursorAndHighlight(intersects: THREE.Intersection[]): void {
    ...
    if (intersects.length > 0) {
      const userData = intersects[0].object.userData as UserData;
