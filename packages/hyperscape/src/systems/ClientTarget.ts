...
export class ClientTarget extends System {
  ...
  show(vec3: THREE.Vector3) {
    ...
    this.bounds = this.ui?.getBoundingClientRect() || null
  }
  ...
  lateUpdate() {
    ...
    const y = ((-vector.y + 1) * (this.bounds?.height || 0)) / 2
    ...
    // If within -1..1 in clip spac(e) => on-screen
    if (vector.x >= -1 && vector.x <= 1 && vector.y >= -1 && vector.y <= 1) {
      ...
      const centerX = (this.bounds?.width || 0) / 2
      const centerY = (this.bounds?.height || 0) / 2
