...
export class ClientInterface extends SystemBase {
  ...
  maxPing: number = 0.01
  ping: any  // Panel type not strictly typed
  ...
  start() {
    // UI control binding
    this.control = (this.world as any).controls?.bind({ priority: ControlPriorities.CORE_UI }) || null
  ...
  update(delta: number) {
    ...
    if (this.control && this.state.visible) {
      const keyB = (this.control as any).keyB
      if (keyB?.pressed) {
        this.toggleActive()
      }
    }
  ...
  broadcast() {
    this.emitTypedEvent('ui-update' as any, { ...this.state })
  }
  ...
  showTarget(vec3: THREE.Vector3) {
    ...
    this.targetBounds = (this.uiContainer as any)?.getBoundingClientRect() || null
  }
  ...
  toggleStats(value?: boolean) {
    ...
    if (this.statsActive) {
      if (!this.statsPanel) {
        ...
        this.ping = new (Panel as any)('PING', '#f00', '#200')
  ...
  onPong(time: number) {
    ...
    if (this.statsActive && this.ping) {
      ...
      ;(this.ping as any).update(rttMs, this.maxPing, graph, 3)
      
      let color = '#0f0'
      if (avgPing > 100) color = '#f00'
      else if (avgPing > 50) color = '#ff0'
      
      ;(this.ping as any).fg = color
      const firstChild = (this.ping as any).dom?.children?.[0] as HTMLElement
