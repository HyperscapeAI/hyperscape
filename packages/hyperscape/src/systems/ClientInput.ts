...
import type { 
  World, WorldOptions, ControlBinding, ControlEntry, ButtonEntry, 
...
class PointerState {
  ...
  update(hit: { node?: PointerNode } | null, pressed: boolean, released: boolean) {
    ...
    for (const node of this.activePath) {
      if (!newPath.has(node)) {
        node.onPointerLeave?.({} as any)
      }
    }
    
    for (const node of newPath) {
      if (!this.activePath.has(node)) {
        node.onPointerEnter?.({} as any)
      }
    }
...
export class ClientInput extends SystemBase {
  ...
  private onPointerMove = (e: PointerEvent) => {
    if ((e as any).isCoreUI || !this.viewport) return
